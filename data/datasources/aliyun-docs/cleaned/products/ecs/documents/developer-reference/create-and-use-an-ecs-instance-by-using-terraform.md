...
## 编写Terraform配置文件
在main.tf中定义创建ECS所需的基础设施资源，例如ECS、VPC等。您可以直接复制下方[完整示例](create-and-use-an-ecs-instance-by-using-terraform.md)中的代码到配置文件中
创建配置文件。
...
## 连接ECS实例
当ECS实例成功创建后，您可以通过SSH协议远程登录到指定公网IP的ECS。更多远程连接方式，请参见[连接方式概述](../user-guide/connect-to-instance.md)。
ssh <用户名>@<公网IP>
## 查看结果
## 使用Terraform命令
...
## 相关文档
ECS支持的Resource和Data Source请参考[支持的资源列表](terraform.md)。
