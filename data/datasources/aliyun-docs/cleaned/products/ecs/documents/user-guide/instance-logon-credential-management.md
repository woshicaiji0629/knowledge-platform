...
## 在线重置密码（不重启）
说明
在线重置密码依赖实例中安装的云助手Agent。[查看云助手状态](check-the-status-of-cloud-assistant-and-handle-exceptions.md)，[安装云助手](install-the-cloud-assistant-agent.md)[Agent](install-the-cloud-assistant-agent.md)。
...
重置密码的方式：在线重置密码。
重要
如无法选择在线重置实例密码，请使用[离线重置密码（需重启）](instance-logon-credential-management.md)。
若提示重置密码[失败](instance-logon-credential-management.md)，请使用[离线重置密码（需重启）](instance-logon-credential-management.md)。
## 离线重置密码（需重启）
重要
...
重置密码的方式：在线重置密码。
重要
如无法选择在线重置实例密码，请[登录实例手动修改密码](instance-logon-credential-management.md)。
若提示重置密码[失败](instance-logon-credential-management.md)，请[登录实例手动修改密码](instance-logon-credential-management.md)。
## 登录实例手动修改密码
## Windows实例
以Windows Server 2019操作系统为例：
[使用](connect-to-a-windows-instance-through-workbench.md)[Workbench](connect-to-a-windows-instance-through-workbench.md)[登录](connect-to-a-windows-instance-through-workbench.md)[Windows](connect-to-a-windows-instance-through-workbench.md)。
...
单击确定，完成导入。
## API
创建密钥对：[CreateKeyPair](../developer-reference/api-ecs-2014-05-26-createkeypair.md)。
导入密钥对中的公钥：[ImportKeyPair](../developer-reference/api-ecs-2014-05-26-importkeypair.md)。
### 2.2 绑定/换绑
## 控制台
说明
...
## API
调用[DeleteKeyPairs](../developer-reference/api-ecs-2014-05-26-deletekeypairs.md)，并指定KeyPairNames为要删除的密钥对名称列表。
## 三、多用户远程登录
在需要设置多个用户使用ECS实例时，请遵循以下步骤，创建普通用户并启用远程访问。
## Linux系统
[使用](connect-to-a-linux-instance-by-using-a-password-or-key.md)[Workbench](connect-to-a-linux-instance-by-using-a-password-or-key.md)[登录实例](connect-to-a-linux-instance-by-using-a-password-or-key.md)，按以下步骤创建用户：
