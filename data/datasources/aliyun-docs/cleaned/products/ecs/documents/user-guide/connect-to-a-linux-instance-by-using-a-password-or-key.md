...
免密登录原理是什么？
免密登录默认采用[临时](workbench-password-free-login-principle-temporary-ssh-key-pair.md)[SSH](workbench-password-free-login-principle-temporary-ssh-key-pair.md)[密钥对](workbench-password-free-login-principle-temporary-ssh-key-pair.md)免密登录；若启用会话管理功能，则优先通过[会话管理](connect-to-an-instance-by-using-session-manager-2.md)链路连接实例。
