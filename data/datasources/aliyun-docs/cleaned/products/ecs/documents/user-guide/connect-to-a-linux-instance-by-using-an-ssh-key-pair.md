...
重要
推荐通过[Workbench](connect-to-a-linux-instance-by-using-a-password-or-key.md)连接阿里云上的实例，该工具可直接通过浏览器使用、支持免密登录，相比使用OpenSSH、Xshell更便捷。
## 适用范围
实例操作系统为Linux。
实例已绑定[固定公网](public-ip-address.md)[IP](public-ip-address.md)或[弹性公网](associate-or-disassociate-an-eip.md)[IP](associate-or-disassociate-an-eip.md)。
...
实例登录凭证：为实例[设置密码](instance-logon-credential-management.md)或[绑定密钥对](instance-logon-credential-management.md)。
配置安全组：为实例关联的安全组[配置入方向规则](connect-to-a-linux-instance-by-using-an-ssh-key-pair.md)，允许本地IP通过SSH（22端口）访问实例。
### 操作步骤
## Windows 10/11
## 密码登录
打开PowerShell
