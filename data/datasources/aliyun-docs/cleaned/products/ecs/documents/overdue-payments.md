...
结清欠费账单后，系统会自动重启因欠费导致停机实例。如果偶尔出现自动重开机失败的情况，您也可以手动启动实例。具体操作，请参见[启动实例](user-guide/start-an-instance.md)。
## 如何避免欠费停服
释放无用资源
...
设置自动释放时间
为按量付费实例开启自动释放功能，避免超出预期开通时长产生不必要的费用或欠费。具体操作，请参见[释放实例](user-guide/release-an-instance.md)。
开启延期免停权益
...
[如果我的账号余额不足，系统会提醒我吗？什么时候提示？](billing-faq.md)
[账号余额不足时，按量付费实例上的数据会受到影响吗？](billing-faq.md)
[购买](billing-faq.md)[ECS](billing-faq.md)[实例后提出了退款，为什么还是扣了费用？](billing-faq.md)
