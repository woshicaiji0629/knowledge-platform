...
## 费用说明
调用SelectObject接口查询数据时，按扫描的原文件实际大小计费。更多信息，请参见[数据处理费用](../data-processing-fees.md)。
## 支持的文件类型
...
## 相关API
以上操作方式底层基于API实现，如果您的程序自定义要求较高，您可以直接发起REST API请求。直接发起REST API请求需要手动编写代码计算签名。更多信息，请参见[SelectObject](../developer-reference/selectobject.md)。
该文章对您有帮助吗？
反馈
### 为什么选择阿里云
