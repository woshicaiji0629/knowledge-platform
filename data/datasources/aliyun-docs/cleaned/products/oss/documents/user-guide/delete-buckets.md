...
## 清理必须删除的资源
删除 Bucket 的前提是清空其内部的所有资源，请根据[系统检测出的剩余资源](delete-buckets.md)进行清理。文件量较大时，推荐[使用生命周期规则](delete-buckets.md)自动处理。
## 清理文件
...
更多示例，请参见[rm（删除）](../developer-reference/rm-deleted.md)。
## HDFS文件
通过OSS控制台的方式
...
更多信息，请参见[rm（删除）](../developer-reference/rm-deleted.md)。
## 清理接入点
...
更多信息，请参见[delete-bucket](../developer-reference/delete-bucket.md)。
## OSS SDK
以下仅列举常见SDK的删除Bucket的代码示例。关于其他SDK的删除Bucket的代码示例，请参见[SDK](../developer-reference/overview-21.md)[简介](../developer-reference/overview-21.md)。
### Java
...
## 常见问题
### 删除时提示“Bucket不为空”怎么办？
Bucket内还有资源未清理。建议通过OSS控制台查看当前剩余资源，然后参照[清理必须删除的资源](delete-buckets.md)进行清理，清理完毕后重试删除Bucket。
### 删除后为何无法立即创建同名Bucket？
