...
| --- | --- | --- | --- |
| 外网流入流量 | 不涉及 | 通过外网 Endpoint（示例值 oss-cn-hangzhou.aliyuncs.com）或者传输加速 Endpoint（示例值 oss-accelerate.aliyuncs.com）调用 PutObject 等上传类接口上传文件产生的流量。 说明 使用传输加速 Endpoint 向 Bucket 上传数据时，还会产生 [传输加速费用](transfer-acceleration-fees.md) 。 | 否 |
...
| 计费项 | 计费项 Code | 计费规则 | 是否计费 |
| --- | --- | --- | --- |
| 跨区域复制流量 | ReplicationDatasize | 使用 [跨区域复制](user-guide/cross-region-replication-overview.md) 功能将源 Bucket 的数据同步复制到目标 Bucket 时产生的流出流量。 跨区域复制流量费用=跨区域复制流量（GB）×每 GB 单价 | 是 |
## 支付方式
### 选型指导
...
资源包不支持抵扣无地域属性的下行流量。
## 计费案例
[标准存储（本地冗余）+数据访问](billing-examples.md)
[标准存储（本地冗余）+跨区域复制](billing-examples.md)
[OSS](billing-examples.md)[结合](billing-examples.md)[CDN](billing-examples.md)[加速服务](billing-examples.md)
## 常见问题
### 外网流出流量出现非预期增长怎么解决？
...
配置CDN加速访问OSS。
如果确认是内容分发引起的下行流量突增，建议使用CDN分发OSS中的图片、视频、文档等静态资源，降低OSS下行流量费用，提升资源加载速度。具体步骤，请参见[通过](user-guide/cdn-acceleration.md)[CDN](user-guide/cdn-acceleration.md)[加速访问](user-guide/cdn-acceleration.md)[OSS](user-guide/cdn-acceleration.md)。
