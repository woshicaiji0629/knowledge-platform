...
解决方案：
[释放存储空间](what-do-i-do-if-my-apsaradb-rds-instance-is-in-the-locking-state.md)
[扩容存储空间](what-do-i-do-if-my-apsaradb-rds-instance-is-in-the-locking-state.md)
### 释放存储空间
...
## 账号欠费或实例到期处理方法
包年包月：如果实例已到期且未续费，为实例[续费](manually-renew-an-apsaradb-rds-for-mysql-instance.md)后，等待5分钟查看实例状态是否为运行中。
...
先将实例规格变更为在售实例规格，然后再扩容实例的存储空间。在售实例规格，请参见[主实例规格列表](../product-overview/primary-apsaradb-rds-instance-types.md)。
