...
## 前提条件
已在服务器上安装Windows Logtail 1.0.0.0及以上版本。具体操作，请参见[安装](install-logtail-on-a-windows-server.md)[Logtail（Windows](install-logtail-on-a-windows-server.md)[系统）](install-logtail-on-a-windows-server.md)。
目标服务器持续产生日志。
重要
...
## 问题排查
使用Logtail采集日志后，如果预览页面或查询页面无数据，您可以参见[Logtail](what-do-i-do-if-errors-occur-when-i-use-logtail-to-collect-logs.md)[采集日志失败的排查思路](what-do-i-do-if-errors-occur-when-i-use-logtail-to-collect-logs.md)进行排查。
## 后续步骤
