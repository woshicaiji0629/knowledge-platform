...
Source: https://help.aliyun.com/zh/ack/ack-managed-and-ack-dedicated/security-and-compliance/work-with-cluster-auditing
...
如果您想在注册集群中实现集群API Server审计功能，请参见[启用集群](../../distributed-cloud-container-platform-for-kubernetes/user-guide/use-cluster-auditing-in-registered-clusters.md)[API Server](../../distributed-cloud-container-platform-for-kubernetes/user-guide/use-cluster-auditing-in-registered-clusters.md)[审计功能](../../distributed-cloud-container-platform-for-kubernetes/user-guide/use-cluster-auditing-in-registered-clusters.md)。
## 计费说明
您可以在费用账单的账单总览页面，查看计费明细，包含审计日志的费用信息。具体操作，请参见[账单查询](../product-overview/view-your-bills.md)。关于审计日志的计费方式，请参见[按使用功能计费](../../../../sls/documents/pay-as-you-go.md)。
...
在资源操作概览页签中，可通过顶部时间范围选择器（如1周相对）和Namespace下拉框筛选数据。页面按资源类型分组，每种资源类型的创建、删除、访问、更新操作以独立环形图面板展示统计。同级页签还包括审计中心概览、资源操作详细列表和Kubernetes CVE 安全风险。
### 资源操作详细列表
...
### Kubernetes CVE安全风险
该报表用于展示当前集群中可能包含的Kubernetes CVE安全风险，您可以选择或输入子账号ID（即RAM用户账号）进行实时查询。该报表会显示当前账号下的Kubernetes CVE安全风险。关于CVE详情和解决方案，请参见[【CVE](../../product-overview/security-bulletins.md)[安全】漏洞修复公告](../../product-overview/security-bulletins.md)。
## （可选）步骤三：查看详细日志记录
...
## 相关文档
如您想使用容器内部的操作审计功能，即通过kubectl exec进入到容器内部执行的命令的审计，请参见[使用容器内部操作审计功能](use-container-auditing.md)。
关于为企业安全管理运维人员提供使用集群的安全最佳实践，请参见[安全最佳实践](best-practices-for-security-1.md)。
[容器安全](faq-about-container-security.md)[FAQ](faq-about-container-security.md)。
该文章对您有帮助吗？
反馈
### 为什么选择阿里云
