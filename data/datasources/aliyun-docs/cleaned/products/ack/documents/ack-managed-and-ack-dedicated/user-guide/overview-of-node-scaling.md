...
请合理配置开启自动伸缩的节点池的最大实例数，保证此范围内的节点所依赖的资源和配额充足，例如合理规划VPC网段、交换机等网络资源，以避免节点扩容失败。配置开启自动伸缩的节点池的最大实例数，请参见[配置实例数量](auto-scaling-of-nodes.md)。关于ACK的网络规划，请参见[ACK](kubernetes-cluster-network-planning.md)[托管集群网络规划](kubernetes-cluster-network-planning.md)。
...
## 后续阅读
根据前文的方案介绍和差异对比，您可以选择[启用节点自动伸缩](auto-scaling-of-nodes.md)、[启用节点即时弹性](instant-elasticity.md)。
如在使用节点伸缩过程中遇到问题，您可参见[节点自动伸缩](faq-about-node-auto-scaling.md)[FAQ](faq-about-node-auto-scaling.md)进行自排查。
展开查看节点自动伸缩的FAQ索引
| 分类 | 二级分类 | 跳转链接 |
| --- | --- | --- |
