...
## 自动扩容实例配置
云数据库 Tair（兼容 Redis）集成了[DAS](https://help.aliyun.com/zh/das/product-overview/what-is-das#concept-2419191)的自动扩容功能，当内存平均使用率达到阈值后会自动升级实例的规格，帮助您快速弹性适配业务高峰，避免内存溢出的风险，有效保障线上业务稳定性。具体操作，请参见[开启自动扩容](enable-automatic-scale-up.md)。
## 常见问题
### 如何查询实例是否扩容过？
您可以在[任务中心](job-center.md)选中状态统计为执行成功的任务，查看是否存在规格变更或迁移的任务，或通过[DescribeHistoryTasks](../developer-reference/api-r-kvstore-2015-01-01-describehistorytasks-redis.md)API接口进行查询。
### 什么原因会导致变配失败？
...
读写分离架构实例增加只读节点数。
集群架构实例增加分片数。
具体操作，请参见[如何升级实例的](../support/how-to-upgrade-redis-cpu-specifications.md)[CPU](../support/how-to-upgrade-redis-cpu-specifications.md)[规格](../support/how-to-upgrade-redis-cpu-specifications.md)。
实例规格信息，请参见[实例规格](../product-overview/overview-4.md)。
...
| API 接口 | 说明 |
| --- | --- |
| [ModifyInstanceSpec](../developer-reference/api-r-kvstore-2015-01-01-modifyinstancespec-redis.md) | 变配实例的规格。 |
该文章对您有帮助吗？
反馈
### 为什么选择阿里云
