...
| Proxy（代理模式）通用异常 | [ERR client ip is not in whitelist](common-errors-and-troubleshooting.md) [NOWRITE You can't write against a non-write redis](common-errors-and-troubleshooting.md) [ERR syntax error](common-errors-and-troubleshooting.md) [ERR no such db node](common-errors-and-troubleshooting.md) [ERR 'xxx' command keys must in same slot](common-errors-and-troubleshooting.md) [ERR for redis cluster, eval/evalsha number of keys can't be negative or zero](common-errors-and-troubleshooting.md) [ERR request refused, too many pending request, now count xxx, beyond threshold xxx](common-errors-and-troubleshooting.md) [ERR redis temporary failure](common-errors-and-troubleshooting.md) [ERR redis temporary failure (ErrorCode 7002)](common-errors-and-troubleshooting.md) |
| Lua 脚本与事务（Transaction） | [NOSCRIPT No matching script. Please use EVAL.](common-errors-and-troubleshooting.md) [BUSY Redis is busy running a script. You can only call SCRIPT KILL or SHUTDOWN NOSAVE.](common-errors-and-troubleshooting.md) [ERR command eval not support for normal user](common-errors-and-troubleshooting.md) [ERR eval/evalsha command keys must be in same slot](common-errors-and-troubleshooting.md) [ERR bad lua script for redis cluster, all the keys that the script uses should be passed using the KEYS array](common-errors-and-troubleshooting.md) [EXECABORT Transaction discarded because of previous errors](common-errors-and-troubleshooting.md) [UNKILLABLE Sorry the script already executed write commands against the dataset.](common-errors-and-troubleshooting.md) [UNKILLABLE The busy script was sent by a master instance in the context of replication and cannot be killed.](common-errors-and-troubleshooting.md) [NOTBUSY No scripts in execution right now.](common-errors-and-troubleshooting.md) |
...
| StackExchange.Redis 客户端 | [Multiple databases are not supported on this server; cannot switch to database](common-errors-and-troubleshooting.md) |
| Predis 客户端 | [Error while reading line from the server.](common-errors-and-troubleshooting.md) |
| phpredis 客户端 | [Cannot assign requested address](common-errors-and-troubleshooting.md) [redis protocol error, got ' ' as reply type byte](common-errors-and-troubleshooting.md) [php_network_getaddresses: getaddrinfo failed: Temporary failure in name resolution](common-errors-and-troubleshooting.md) |
| Go-redis 客户端 | [panic: got 4 elements in cluster info address, expected 2 or 3](common-errors-and-troubleshooting.md) |
| node-redis 客户端 | [SCAN](common-errors-and-troubleshooting.md) [命令死循环或者返回数据为空](common-errors-and-troubleshooting.md) |
## Redis通用异常
### ERR illegal address
可能原因：未将客户端的IP地址添加至Tair实例的白名单中。
解决方法：将客户端的IP地址添加入至Tair实例的白名单中，具体操作请参见[连接诊断](../user-guide/perform-diagnostics-on-connections.md)。
### ERR sentinel compatibility mode is disabled
可能原因：未开启Tair实例的Sentinel兼容。
解决方法：在控制台开启Sentinel兼容，具体操作请参见[开启](../user-guide/enable-the-sentinel-compatible-mode.md)[Sentinel](../user-guide/enable-the-sentinel-compatible-mode.md)[兼容](../user-guide/enable-the-sentinel-compatible-mode.md)。
...
解决方法：检查应用侧代码或调整客户端Buffer的大小，更多信息请参见[Unexpected end of stream](common-errors-and-troubleshooting.md)章节。
### UnknownHostException
或failed to connect: xxx.redis.rds.aliyuncs.com could not be resolved。
可能原因：客户端无法正常解析Tair实例的域名地址。
解决方案：请设置正确的DNS服务器地址，更多信息请参见[解决因域名解析失败导致的连接问题](troubleshoot-connection-issues-caused-by-failed-dns-resolution.md)。
...
可能原因：未正确配置白名单。
解决方案：将客户端的IP地址添加至实例的白名单中，具体操作请参见[设置](../user-guide/configure-whitelists.md)[IP](../user-guide/configure-whitelists.md)[白名单](../user-guide/configure-whitelists.md)。
说明
...
解决方法：对账号进行充值，或对到期的实例进行续费，更多信息请参见[到期与欠费](../product-overview/expiration-and-overdue-payments.md)。
### ERR syntax error
...
解决方法：传入正确的db node，db node需小于分片数量，更多信息请参见[阿里云自研的](../developer-reference/in-house-commands-for-tair-instances-in-proxy-mode.md)[Proxy](../developer-reference/in-house-commands-for-tair-instances-in-proxy-mode.md)[命令](../developer-reference/in-house-commands-for-tair-instances-in-proxy-mode.md)。
...
可能原因：无法执行EVAL的相关命令。
解决方法：请将实例的小版本升级至最新，具体操作请参见[升级小版本与代理版本](../user-guide/update-the-minor-version.md)。
...
可能原因：客户端连接Tair实例超时。
解决方法：更多信息请参见[连接问题排查流程](how-do-i-troubleshoot-connection-issues-in-apsaradb-for-redis.md)。
...
解决方法：若是首次访问Tair实例，请检查是否将客户端的IP地址添加至Tair白名单中或客户端的网络情况；若不是首次访问Tair实例，可以进行[实例诊断](../user-guide/create-a-diagnostic-report.md)，进行问题定位。
