...
重要
本文仅包含Redis开源版实例的小版本发布日志，关于Tair（企业版）实例的小版本发布日志（包含内存型、持久内存型和磁盘型）请参见[Tair](apsaradb-for-redis-enhanced-edition-1.md)[小版本发布日志](apsaradb-for-redis-enhanced-edition-1.md)。
## 如何查询或升级实例的小版本
...
| 6.0.1.1 | MEDIUM | 2020-11-28 | 新特性 | 合并社区 6.0.9 版本的更新内容，更多信息，请参见 [Redis 6.0 release notes](https://raw.githubusercontent.com/redis/redis/6.0/00-RELEASENOTES) 。 支持槽（slot）的无感迁移能力。 支持通过公网获取虚拟 IP（VIP）地址，为使用 [直连模式](../user-guide/enable-the-direct-connection-mode.md) 客户端提供更好的支持。 |
...
| 5.1.9 | LOW | 2022-06-22 | 功能优化 | 优化延迟统计直方图，详情请参见 [时延洞察](../user-guide/latency-insights.md) 。 |
| 安全加固 | 升级、优化集群架构增、删节点流程，详情请参见 [调整集群分片数](../user-guide/adjust-the-number-of-cluster-shards.md) 。 |  |  |  |
...
| 5.1.4 | LOW | 2022-03-24 | 新特性 | 支持延时统计直方图（Latency），详情请参见 [时延洞察](../user-guide/latency-insights.md) 。 支持 INFO 命令返回 Errorstats（Redis 错误统计）信息。 |
| 5.1.3 | LOW | 2022-02-21 | 功能优化 | 支持 READONLY、READWRITE 命令，详情请参见 [Redis](../developer-reference/commands-supported-by-apsaradb-for-redis-community-edition.md) [开源版命令支持](../developer-reference/commands-supported-by-apsaradb-for-redis-community-edition.md) 。 |
...
## 相关文档
Proxy节点的版本发布记录请参见[Proxy](apsaradb-for-redis-proxy-nodes.md)[小版本发布日志](apsaradb-for-redis-proxy-nodes.md)。
Tair（企业版）的版本发布记录请参见[Tair](apsaradb-for-redis-enhanced-edition-1.md)[小版本发布日志](apsaradb-for-redis-enhanced-edition-1.md)。
该文章对您有帮助吗？
反馈
### 为什么选择阿里云
