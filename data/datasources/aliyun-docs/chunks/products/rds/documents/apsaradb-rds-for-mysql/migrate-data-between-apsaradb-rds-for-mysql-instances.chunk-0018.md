-is-dms#task-1919582) 执行的原始 DDL 语句。 说明 该方案会导致目标库锁表。 否，适配 gh-ost ：不迁移 Online DDL 变更产生的临时表数据，只迁移源库使用 gh-ost 执行的原始 DDL 语句，同时您可以使用默认的或者自行配置 gh-ost 影子表和无用表的正则表达式。 说明 该方案会导致目标库锁表。 |
...
| 是否去除正反向任务的心跳表 sql | 根据业务需求选择是否在 DTS 实例运行时，在源库中写入心跳 SQL 信息。 是 ：不在源库中写入心跳 SQL 信息，DT
