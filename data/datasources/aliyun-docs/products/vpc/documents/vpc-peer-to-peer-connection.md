...
当有大量 VPC 互连、大带宽、低成本的综合需求时，可同时使用 VPC 对等连接和云企业网。二者区别，可参考[VPC](products/vpc/documents/cross-vpc-interconnection-overview.md)[互连](products/vpc/documents/cross-vpc-interconnection-overview.md)。
...
接收端也可以拒绝或删除连接请求，可以参考[VPC](products/vpc/documents/vpc-peer-to-peer-connection.md)[对等连接的状态机](products/vpc/documents/vpc-peer-to-peer-connection.md)了解完整流程。
...
调用[CreateVpcPeerConnection](products/vpc/documents/developer-reference/api-vpcpeer-2022-01-01-createvpcpeerconnection.md)创建VPC对等连接。

- 

两端VPC属于不同账号时，需使用接收端账号调用[AcceptVpcPeerConnection](products/vpc/documents/developer-reference/api-vpcpeer-2022-01-01-acceptvpcpeerconnection.md)接受VPC对等连接。

接收端可以调用[RejectVpcPeerConnection](products/vpc/documents/developer-reference/api-vpcpeer-2022-01-01-rejectvpcpeerconnection.md)拒绝VPC对等连接。

- 

使用两端 VPC 所属账号，分别调用[GetVpcPeerConnectionAttribute](products/vpc/documents/developer-reference/api-vpcpeer-2022-01-01-getvpcpeerconnectionattribute.md)查询两端VPC的网段。

- 

使用两端 VPC 所属账号，分别调用[CreateRouteEntry](products/vpc/documents/developer-reference/api-vpc-2016-04-28-createrouteentry.md)创建指向对等连接的路由条目。

修改跨地域对等连接

调用[ModifyVpcPeerConnection](products/vpc/documents/developer-reference/api-vpcpeer-2022-01-01-modifyvpcpeerconnection.md)修改跨地域VPC对等连接的带宽或链路类型。

删除对等连接

- 

调用[DeleteRouteEntry](products/vpc/documents/developer-reference/api-vpc-2016-04-28-deleterouteentry.md)删除指向对等连接的路由条目。

- 

调用[DeleteVpcPeerConnection](products/vpc/documents/developer-reference/api-vpcpeer-2022-01-01-deletevpcpeerconnection.md)删除VPC对等连接。
...
建议优先使用[路径分析](products/vpc/documents/vpc-peer-to-peer-connection.md)验证网络连通性。
...
| 网段配置 | 查看发起端网段和接收端网段： 是否重叠。 是否使用了非 [RFC 1918](https://www.rfc-editor.org/rfc/rfc1918) 标准私网网段（如 198.19.0.0/16、30.0.0.0/8 等）。VPC 将这些网段视为公网地址，ECS 绑定公网 IP 时流量默认走公网出口。 当 ECS 实例部署 Docker 时，是否与 Docker 网卡地址冲突。 | 网段重叠时，需将业务迁移到网段不重叠的 VPC，重新建立对等连接。 使用非 RFC 标准私网网段时， [使用](products/vpc/documents/using-ipv4-gateway-for-public-network-private-use.md) [IPv4](products/vpc/documents/using-ipv4-gateway-for-public-network-private-use.md) [网关实现公网私用](products/vpc/documents/using-ipv4-gateway-for-public-network-private-use.md) ，确保流量能够正确地到达目标 VPC。 [修改 Docker 网段](products/vpc/documents/vpc-peer-to-peer-connection.md) 。 |
| 路由配置 | 查看对等连接详情页的 路由条目列表 ： 是否为两端 VPC 都配置了指向对方的路由。 目标网段是否正确填写对端的 VPC 网段。 路由条目是否添加到资源所在交换机绑定的路由表。 | 检查并修正 [双向路由配置](products/vpc/documents/vpc-peer-to-peer-connection.md) 。 |
...
VPC 将[RFC 1918](https://www.rfc-editor.org/rfc/rfc1918)之外的 IP 地址空间（如 198.19.0.0/16、30.0.0.0/16 等）视为公网网段。当 VPC 中的 ECS 绑定了公网 IP 或通过 NAT 网关具备公网访问能力时，访问这些网段的流量会走公网出口，无法通过对等连接到达目标 VPC。需要在 VPC 中[使用](products/vpc/documents/using-ipv4-gateway-for-public-network-private-use.md)[IPv4](products/vpc/documents/using-ipv4-gateway-for-public-network-private-use.md)[网关实现公网私用](products/vpc/documents/using-ipv4-gateway-for-public-network-private-use.md)，将目标公网网段添加为私网路由，确保流量通过对等连接正确路由到目标 VPC。
...
[将策略授权给目标 RAM 用户、用户组或角色](products/ram/documents/user-guide/grant-permissions-to-the-ram-user.md)。

API

- 

调用[CreatePolicy](products/ram/documents/developer-reference/api-ram-2015-05-01-createpolicy.md)创建自定义策略，将上方所选方案的策略作为PolicyDocument参数传入。

- 

调用[AttachPolicyToUser](products/ram/documents/developer-reference/api-ram-2015-05-01-attachpolicytouser.md)、[AttachPolicyToGroup](products/ram/documents/developer-reference/api-ram-2015-05-01-attachpolicytogroup.md)或[AttachPolicyToRole](products/ram/documents/developer-reference/api-ram-2015-05-01-attachpolicytorole.md)将策略授予目标 RAM 用户、用户组或角色。
...
[上一篇：VPC互连](products/vpc/documents/cross-vpc-interconnection-overview.md)[下一篇：VPC连接本地数据中心/其他云](products/vpc/documents/connect-vpc-to-local-idc-office-terminal-other-cloud.md)
