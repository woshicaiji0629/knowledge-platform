...
合理的网络规划需要避免网段冲突并保障网络的可扩展性，网络规划不当将会导致后期极高的重建成本。因此，建议在创建专有网络之前先进行[网络规划](products/vpc/documents/vpc-network-planning.md)。
...
IPv4网段：选择控制台提供的建议网段，或根据需要输入自定义网段。在多VPC互通等场景下，建议配置与已有VPC不重叠的网段来避免VPC互通时的网段冲突问题。为了避免网段冲突、保障网络的可扩展性，建议[结合](products/vpc/documents/vpc-and-vswitch.md)[IPAM](products/vpc/documents/vpc-and-vswitch.md)[创建专有网络](products/vpc/documents/vpc-and-vswitch.md)。
...
依次调用[CreateVpc](products/vpc/documents/developer-reference/api-vpc-2016-04-28-createvpc.md)和[CreateVSwitch](products/vpc/documents/developer-reference/api-vpc-2016-04-28-createvswitch.md)创建专有网络和交换机。

- 

依次调用[DeleteVSwitch](products/vpc/documents/developer-reference/api-vpc-2016-04-28-deletevswitch.md)和[DeleteVpc](products/vpc/documents/developer-reference/api-vpc-2016-04-28-deletevpc.md)删除交换机和专有网络。
...
[支持](products/vpc/documents/regions-that-support-vpc-features.md)[IPv4/IPv6](products/vpc/documents/regions-that-support-vpc-features.md)[双栈的地域](products/vpc/documents/regions-that-support-vpc-features.md)。
...
创建专有网络和交换机时，调整[CreateVpc](products/vpc/documents/developer-reference/api-vpc-2016-04-28-createvpc.md)和[CreateVSwitch](products/vpc/documents/developer-reference/api-vpc-2016-04-28-createvswitch.md)的EnableIPv6参数开启/关闭IPv6。创建专有网络时，传入Ipv6IpamPoolId和Ipv6CidrMask将从指定的IPv6地址池中为VPC分配IPv6网段。

- 

针对已创建的专有网络和交换机，调整[ModifyVpcAttribute](products/vpc/documents/developer-reference/api-vpc-2016-04-28-modifyvpcattribute.md)与[ModifyVSwitchAttribute](products/vpc/documents/developer-reference/api-vpc-2016-04-28-modifyvswitchattribute.md)的EnableIPv6参数开启/关闭IPv6。如需从指定IPv6地址池中为VPC分配IPv6网段，调用[AssociateVpcCidrBlock](products/vpc/documents/developer-reference/api-vpc-2016-04-28-associatevpccidrblock.md)添加。
...
创建专有网络时，为其指定的IPv4网段是专有网络的主网段。控制台不支持修改专有网络的主网段，但可以调整[ModifyVpcAttribute](products/vpc/documents/developer-reference/api-vpc-2016-04-28-modifyvpcattribute.md)接口的CidrBlock参数，在主网段内放大或缩小网段。需确保缩小后的网段包含已经使用的IP地址。
...
1、不能使用100.64.0.0/10、224.0.0.0/4、127.0.0.0/8或169.254.0.0/16网段作为IPv4附加网段。2、附加网段的地址块不能与主网段重叠。3、每个专有网络默认支持添加[5](products/vpc/documents/understanding-vpc-quotas-in-alibaba-cloud.md)[个](products/vpc/documents/understanding-vpc-quotas-in-alibaba-cloud.md)[IPv4](products/vpc/documents/understanding-vpc-quotas-in-alibaba-cloud.md)[附加网段，5](products/vpc/documents/understanding-vpc-quotas-in-alibaba-cloud.md)[个](products/vpc/documents/understanding-vpc-quotas-in-alibaba-cloud.md)[IPv6](products/vpc/documents/understanding-vpc-quotas-in-alibaba-cloud.md)[附加网段](products/vpc/documents/understanding-vpc-quotas-in-alibaba-cloud.md)。
...
IPAM 分配的 IPv4 地址段：使用[IPAM](products/vpc/documents/ip-address-management-ipam.md)可以避免分配的网段出现冲突。如果已有预置CIDR的IPAM地址池，建议选择此项。配置时，首先选择地址池，再配置IPv4地址掩码。
...
调用[AssociateVpcCidrBlock](products/vpc/documents/developer-reference/api-vpc-2016-04-28-associatevpccidrblock.md)添加附加网段。

- 

调用[UnassociateVpcCidrBlock](products/vpc/documents/developer-reference/api-vpc-2016-04-28-unassociatevpccidrblock.md)删除附加网段。
...
在交换机中预留网段，可以确保此网段不被其他资源的创建所占用。预留的网段，当前仅用于给弹性网卡的辅助私网IP分配[IP](products/ecs/documents/user-guide/ip-prefix.md)[前缀](products/ecs/documents/user-guide/ip-prefix.md)。

1、预留网段内不能包含交换机的[系统保留地址](products/vpc/documents/vpc-and-vswitch.md)。2、单个 VPC 支持创建的 IPv4、IPv6最大预留网段数目均为100个。3、IPv4预留网段的掩码长度最大不超过28，IPv6网段掩码长度且最大不超过80。
...
调用[CreateVSwitchCidrReservation](products/vpc/documents/developer-reference/api-vpc-2016-04-28-createvswitchcidrreservation.md)创建预留网段。

- 

调用[GetVSwitchCidrReservationUsage](products/vpc/documents/developer-reference/api-vpc-2016-04-28-getvswitchcidrreservationusage.md)查看已占用IP段。

- 

调用[DeleteVSwitchCidrReservation](products/vpc/documents/developer-reference/api-vpc-2016-04-28-deletevswitchcidrreservation.md)删除预留网段。
...
IPAM 是云上 IP 地址管理的工具，可以自动化分配与管理 IP 地址，简化网络管理流程并避免地址冲突。[结合](products/vpc/documents/use-ipam-to-create-a-vpc.md)[IPAM](products/vpc/documents/use-ipam-to-create-a-vpc.md)[规划](products/vpc/documents/use-ipam-to-create-a-vpc.md)，创建IPAM和IPAM地址池后，可以从IPAM地址池为专有网络分配IPv4网段和IPv6网段。
...
分配IPv4网段：可在调用[CreateVpc](products/vpc/documents/developer-reference/api-vpc-2016-04-28-createvpc.md)时传入Ipv4IpamPoolId来指定IPAM地址池，同时传入Ipv4CidrMask来指定掩码从IPAM地址池中分配地址作为新建VPC的IPv4网段。也可以传入CidrBlock来指定VPC使用的网段，而不是通过指定掩码自动分配网段。
...
[OpenVpcIpamService-开通](products/vpc/documents/developer-reference/api-vpcipam-2023-02-28-openvpcipamservice.md)[IPAM](products/vpc/documents/developer-reference/api-vpcipam-2023-02-28-openvpcipamservice.md)[功能](products/vpc/documents/developer-reference/api-vpcipam-2023-02-28-openvpcipamservice.md)

- 

[CreateIpam-创建一个](products/vpc/documents/developer-reference/api-vpcipam-2023-02-28-createipam.md)[IPAM](products/vpc/documents/developer-reference/api-vpcipam-2023-02-28-createipam.md)[实例](products/vpc/documents/developer-reference/api-vpcipam-2023-02-28-createipam.md)

- 

[CreateIpamPool-创建](products/vpc/documents/developer-reference/api-vpcipam-2023-02-28-createipampool.md)[IPAM](products/vpc/documents/developer-reference/api-vpcipam-2023-02-28-createipampool.md)[地址池](products/vpc/documents/developer-reference/api-vpcipam-2023-02-28-createipampool.md)

- 

[AddIpamPoolCidr-为](products/vpc/documents/developer-reference/api-vpcipam-2023-02-28-addipampoolcidr.md)[IPAM](products/vpc/documents/developer-reference/api-vpcipam-2023-02-28-addipampoolcidr.md)[地址池预置](products/vpc/documents/developer-reference/api-vpcipam-2023-02-28-addipampoolcidr.md)[CIDR](products/vpc/documents/developer-reference/api-vpcipam-2023-02-28-addipampoolcidr.md)[地址段](products/vpc/documents/developer-reference/api-vpcipam-2023-02-28-addipampoolcidr.md)
...
可以在尚未创建默认专有网络的地域，调用[CreateDefaultVpc](products/vpc/documents/developer-reference/api-vpc-2016-04-28-createdefaultvpc.md)和[CreateDefaultVSwitch](products/vpc/documents/developer-reference/api-vpc-2016-04-28-createdefaultvswitch.md)创建默认专有网络和交换机。此方式创建的默认专有网络的网段为172.xx.0.0/16。
