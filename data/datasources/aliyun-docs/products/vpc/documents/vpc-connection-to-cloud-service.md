...
注意网关终端节点[仅在部分地域支持](products/vpc/documents/vpc-connection-to-cloud-service.md)。
...
网关终端节点完成创建后，系统会自动在选中的路由表里，增加一个自定义路由条目：目标网段为一个系统前缀列表（里面包含[OSS 在该地域的 VIP 网段](products/oss/documents/user-guide/regions-and-endpoints.md)），下一跳为创建的网关终端节点。

- 

配置终端节点策略：语法与访问控制RAM产品的[权限策略语言](products/ram/documents/policy-elements.md)相同。
...
配置Bucket授权策略：语法与访问控制RAM产品的[权限策略语言](products/ram/documents/policy-elements.md)相同。
...
OSS的Action汇总请参见[RAM Policy](products/oss/documents/ram-policy-overview.md)[概述](products/oss/documents/ram-policy-overview.md)。Deny策略建议尽量避免配置Action为*，以免导致Bucket所有者在OSS控制台也无法访问Bucket。
...
创建网关终端节点并配置终端节点策略：调用[CreateVpcGatewayEndpoint](products/vpc/documents/developer-reference/api-vpc-2016-04-28-createvpcgatewayendpoint.md)接口。

- 

创建时需要传入终端节点服务名称字段ServiceName，可以通过调用[ListVpcEndpointServicesByEndUser](products/vpc/documents/developer-reference/api-vpc-2016-04-28-listvpcendpointservicesbyenduser.md)接口，查询可使用的终端节点服务。

- 

PolicyDocument字段用于配置终端节点策略，语法与访问控制RAM产品的[权限策略语言](products/ram/documents/policy-elements.md)相同。

- 

修改网关终端节点策略：调用[UpdateVpcGatewayEndpointAttribute](products/vpc/documents/developer-reference/api-vpc-2016-04-28-updatevpcgatewayendpointattribute.md)接口，传入PolicyDocument字段。

- 

绑定路由表：调用[AssociateRouteTablesWithVpcGatewayEndpoint](products/vpc/documents/developer-reference/api-vpc-2016-04-28-associateroutetableswithvpcgatewayendpoint.md)接口。

- 

解绑路由表：调用[DissociateRouteTablesFromVpcGatewayEndpoint](products/vpc/documents/developer-reference/api-vpc-2016-04-28-dissociateroutetablesfromvpcgatewayendpoint.md)接口。

- 

删除网关终端节点：调用[DeleteVpcGatewayEndpoint](products/vpc/documents/developer-reference/api-vpc-2016-04-28-deletevpcgatewayendpoint.md)接口。
...
配置OSS的bucket授权策略：调用[PutBucketPolicy](products/oss/documents/developer-reference/putbucketpolicy.md)接口。

- 

修改Bucket授权策略：调用[PutBucketPolicy](products/oss/documents/developer-reference/putbucketpolicy.md)接口，传入JSON形式的权限策略。

- 

删除Bucket授权策略：调用[DeleteBucketPolicy](products/oss/documents/developer-reference/deletebucketpolicy.md)接口。
...
访问网络虚拟设备：[通过](products/slb/documents/gateway-based-load-balancing-gwlb/getting-started/gwlb-quickly-implements-load-balancing-for-ipv4-services.md)[GWLB](products/slb/documents/gateway-based-load-balancing-gwlb/getting-started/gwlb-quickly-implements-load-balancing-for-ipv4-services.md)[快速实现](products/slb/documents/gateway-based-load-balancing-gwlb/getting-started/gwlb-quickly-implements-load-balancing-for-ipv4-services.md)[IPv4](products/slb/documents/gateway-based-load-balancing-gwlb/getting-started/gwlb-quickly-implements-load-balancing-for-ipv4-services.md)[流量的安全检测](products/slb/documents/gateway-based-load-balancing-gwlb/getting-started/gwlb-quickly-implements-load-balancing-for-ipv4-services.md)。
...
[上一篇：VPC连接本地数据中心/其他云](products/vpc/documents/connect-vpc-to-local-idc-office-terminal-other-cloud.md)[下一篇：网络隔离](products/vpc/documents/network-isolation.md)
