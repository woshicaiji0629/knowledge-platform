...
您可参考[流日志字段说明](products/vpc/documents/vpc-flow-logs.md)了解所有字段及其含义。
...
流日志不支持采集[组播](products/cen/documents/user-guide/multicast-overview.md)流量。
...
开启流日志分析报表功能：建议勾选。该功能会自动在流日志对应的LogStore上[创建索引](products/sls/documents/create-indexes.md)并[创建仪表盘](products/sls/documents/dashboard-overview.md)，以支持对流日志执行SQL与可视化分析。开启后SLS产品会[产生计费](products/sls/documents/billable-items.md)。
...
成功创建流日志后，系统会自动启动流量信息采集，下一步可以[分析流日志](products/vpc/documents/vpc-flow-logs.md)。

启动或停止流日志

前往专有网络控制台[流日志页面](https://vpc.console.aliyun.com/flowlog/cn-hangzhou/flowlogs)，在目标流日志的操作列单击启动或停止。

停止后，VPC不再收取流日志生成费，但SLS会针对已保存的流日志持续[计费](products/vpc/documents/vpc-flow-logs.md)。

删除流日志

前往专有网络控制台[流日志页面](https://vpc.console.aliyun.com/flowlog/cn-hangzhou/flowlogs)，在目标流日志的操作列单击删除。

删除后，VPC不再收取流日志生成费，但SLS会针对已保存的流日志持续[计费](products/vpc/documents/vpc-flow-logs.md)，需至日志服务控制台[删除对应的](products/sls/documents/manage-a-logstore.md)[Logstore](products/sls/documents/manage-a-logstore.md)才能停止全部费用。
...
调用[OpenFlowLogService](products/vpc/documents/developer-reference/api-vpc-2016-04-28-openflowlogservice.md)开通流日志功能。

- 

调用[CreateProject](products/sls/documents/developer-reference/api-sls-2020-12-30-createproject.md)创建Project，调用[CreateLogStore](products/sls/documents/developer-reference/api-sls-2020-12-30-createlogstore.md)创建Logstore。

确保满足上述条件后，您可以：

- 

调用[CreateFlowLog](products/vpc/documents/developer-reference/api-vpc-2016-04-28-createflowlog.md)创建流日志，可选调用[CreateIndex](products/sls/documents/developer-reference/api-sls-2020-12-30-createindex.md)创建索引。

- 

调用[DeactiveFlowLog](products/vpc/documents/developer-reference/api-vpc-2016-04-28-deactiveflowlog.md)停止流日志。

- 

调用[ActiveFlowLog](products/vpc/documents/developer-reference/api-vpc-2016-04-28-activeflowlog.md)启动流日志。

- 

调用[DeleteFlowLog](products/vpc/documents/developer-reference/api-vpc-2016-04-28-deleteflowlog.md)删除流日志。
...
输入语句[查询分析流日志](products/sls/documents/quick-guide-to-query-and-analysis.md)。
...
自定义查询：可以自行[查询与分析快速指引](products/sls/documents/quick-guide-to-query-and-analysis.md)。

- 

开启域间分析（可选）：在Flowlog详情页面，单击网段设置，在网段设置页签，打开开启“域间分析”开关。

开启域间分析功能后，系统将自动创建数据加工任务，生成具有网段信息的VPC流日志，用于分析不同网段之间的流量情况。数据加工功能会[收取一定的费用](products/sls/documents/billable-items.md)。
...
自定义查询：可以自行[查询和分析具有网段信息的](products/sls/documents/quick-guide-to-query-and-analysis.md)[VPC](products/sls/documents/quick-guide-to-query-and-analysis.md)[流日志](products/sls/documents/quick-guide-to-query-and-analysis.md)。

### API

调用[GetLogsV2](products/sls/documents/developer-reference/api-sls-2020-12-30-getlogsv2.md)查询分析流日志。
...
日志服务的服务费：流日志投递到SLS之后产生，由SLS产品计费。收取[写入和存储等费用](products/sls/documents/billing-overview.md)。
...
可在日志服务控制台[查看现有](products/sls/documents/manage-a-logstore.md)[Logstore](products/sls/documents/manage-a-logstore.md)[的数据保存时间](products/sls/documents/manage-a-logstore.md)，并按需修改。

等保（信息安全等级保护）要求网络日志，如何查询？

阿里云 VPC 默认不记录网络日志。若需满足等保要求，[开启](products/vpc/documents/vpc-flow-logs.md)[VPC](products/vpc/documents/vpc-flow-logs.md)[流日志功能](products/vpc/documents/vpc-flow-logs.md)即可记录并分析出入弹性网卡的流量信息，实现安全合规监控。

[上一篇：运维监控](products/vpc/documents/operation-and-maintenance-monitoring.md)[下一篇：流量镜像](products/vpc/documents/traffic-mirroring-overview.md)
