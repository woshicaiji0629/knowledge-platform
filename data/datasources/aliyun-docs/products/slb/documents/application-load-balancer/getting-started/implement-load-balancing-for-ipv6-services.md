...
双栈支持的地域，请参见[ALB](products/slb/documents/application-load-balancer/user-guide/alb-instance-overview.md)[双栈支持的地域](products/slb/documents/application-load-balancer/user-guide/alb-instance-overview.md)。
...
双栈ALB实例支持将IPv4和IPv6的客户端流量转发至IPv4、IPv6的后端服务。详情请参见[ALB](products/slb/documents/application-load-balancer/user-guide/alb-instance-overview.md)[实例概述](products/slb/documents/application-load-balancer/user-guide/alb-instance-overview.md)。
...
如果计划将ALB部署在交换机VSW1和VSW2，需注意：[ALB](products/slb/documents/product-overview/alb.md)[升级实例](products/slb/documents/product-overview/alb.md)会从每个指定的交换机中分配3个IP地址，包含1个VIP（对外提供服务）和2个Local IP（用于与后端服务器通信），如果IP不足会出现报错并且无法创建实例，请确保交换机VSW1和VSW2中已预留足够的可用IP；升级前的ALB实例不受该限制。
...
在云服务器ECS购买页面的自定义购买页签下，创建2台ECS实例，并将IPv4 ECS修改实例名称为ECS01，将IPv6 ECS修改实例名称为ECS02，两台ECS实例绑定的安全组均需要放行80端口。具体操作，请参见[自定义购买实例](products/ecs/documents/user-guide/create-an-instance-by-using-the-wizard.md)。
...
远程登录ECS01和ECS02实例，具体操作，请参见[ECS](products/ecs/documents/user-guide/connect-to-instance.md)[远程连接操作指南](products/ecs/documents/user-guide/connect-to-instance.md)。
...
如果未返回inet6相关内容，表示实例未开启IPv6服务，请先[开启](products/ecs/documents/user-guide/step-1-create-a-vpc-that-supports-ipv6-addressing-step-1-create-a-vpc-that-supports-ipv6-addressing.md)[IPv6](products/ecs/documents/user-guide/step-1-create-a-vpc-that-supports-ipv6-addressing-step-1-create-a-vpc-that-supports-ipv6-addressing.md)[服务](products/ecs/documents/user-guide/step-1-create-a-vpc-that-supports-ipv6-addressing-step-1-create-a-vpc-that-supports-ipv6-addressing.md)。

- 

如果返回inet6相关内容，表示ECS02实例已成功开启IPv6服务并识别到IPv6地址，请继续[配置](products/ecs/documents/user-guide/step-1-create-a-vpc-that-supports-ipv6-addressing-step-1-create-a-vpc-that-supports-ipv6-addressing.md)[IPv6](products/ecs/documents/user-guide/step-1-create-a-vpc-that-supports-ipv6-addressing-step-1-create-a-vpc-that-supports-ipv6-addressing.md)[地址](products/ecs/documents/user-guide/step-1-create-a-vpc-that-supports-ipv6-addressing-step-1-create-a-vpc-that-supports-ipv6-addressing.md)。
...
此处仅列出和本文强相关的配置项，其他未列出的配置项使用默认值。关于参数的更多信息，请参见[创建和管理](products/slb/documents/application-load-balancer/user-guide/create-and-manage-alb-instances.md)[ALB](products/slb/documents/application-load-balancer/user-guide/create-and-manage-alb-instances.md)[实例](products/slb/documents/application-load-balancer/user-guide/create-and-manage-alb-instances.md)。
...
| 实例网络类型 | 选择实例网络类型，系统会根据您的选择分配私网或公网服务地址。本文选择 公网 。 说明 实例网络类型 选择 公网 类型只作用于 IPv4，IPv6 默认是私网类型。本文使用 IPv6 的公网类型，需执行 [步骤](products/slb/documents/application-load-balancer/getting-started/implement-load-balancing-for-ipv6-services.md) [4](products/slb/documents/application-load-balancer/getting-started/implement-load-balancing-for-ipv6-services.md) 变更 IPv6 的网络类型为公网类型。 |
...
| 实例名称 | 输入自定义实例名称。 |
| 服务关联角色 | 首次创建应用型负载均衡实例时，需要单击 创建服务关联角色 ，创建一个名称为 AliyunServiceRoleForAlb 的服务关联角色。系统会为该角色添加名称为 AliyunServiceRolePolicyForAlb 的权限策略，授予 ALB 拥有访问其他云产品实例的权限。更多操作，请参见 [负载均衡系统权限策略参考](products/slb/documents/security-and-compliance/application-oriented-load-balancing-system-permission-policy-reference.md) 。 |


- 

创建完公网双栈ALB实例后，本文需要使用公网IPv6地址，请执行以下步骤将ALB实例的IPv6地址变更为公网地址。更多信息，请参见[协议版本](products/slb/documents/application-load-balancer/user-guide/change-the-network-type-of-an-alb-instance.md)。
...
此处仅列出和本文强相关的配置项，其他未列出的配置项使用默认值。关于参数的更多信息，请参见[创建/删除服务器组](products/slb/documents/application-load-balancer/user-guide/create-and-manage-a-server-group.md)。
...
此处仅列出和本文强相关的配置项，其他未列出的配置项使用默认值。关于配置监听的更多操作，请参见[添加](products/slb/documents/add-an-http-listener.md)[HTTP](products/slb/documents/add-an-http-listener.md)[监听](products/slb/documents/add-an-http-listener.md)。
...
移除后端服务器，具体操作，请参见[创建和管理服务器组](products/slb/documents/application-load-balancer/user-guide/create-and-manage-a-server-group.md)。

- 

删除服务器组，具体操作，请参见[创建和管理服务器组](products/slb/documents/application-load-balancer/user-guide/create-and-manage-a-server-group.md)。
...
了解ALB的应用场景、组成等信息，请参见[什么是应用型负载均衡](products/slb/documents/application-load-balancer/product-overview/what-is-alb.md)[ALB](products/slb/documents/application-load-balancer/product-overview/what-is-alb.md)。

- 

了解ALB的功能特性，请参见[功能特性](products/slb/documents/application-load-balancer/product-overview/functional-characteristics.md)。

- 

了解ALB的配额及提升配额方式，请参见[配额与限制](products/slb/documents/application-load-balancer/product-overview/quotas-and-limits.md)。

- 

了解ALB支持的地域信息，请参见[ALB](products/slb/documents/application-load-balancer/product-overview/supported-regions-and-zones.md)[支持的地域与可用区](products/slb/documents/application-load-balancer/product-overview/supported-regions-and-zones.md)。

- 

了解ALB付费方式、计费组成和定价等计费信息，请参见[ALB](products/slb/documents/application-load-balancer/product-overview/billing-overview.md)[计费概述](products/slb/documents/application-load-balancer/product-overview/billing-overview.md)。

[上一篇：ALB快速实现IPv4服务的负载均衡](products/slb/documents/application-load-balancer/getting-started/use-an-alb-instance-to-provide-ipv4-services.md)[下一篇：操作指南](products/slb/documents/application-load-balancer/user-guide.md)
