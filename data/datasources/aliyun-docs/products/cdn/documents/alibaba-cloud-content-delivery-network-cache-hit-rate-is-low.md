...
如果发生命中率过低的情况意味用户的每次请求都会通过CDN回源，公网链路的不稳定可能导致加速的效果反而变差。如何解决命中率过低的问题，可以通过预热URL、配置资源缓存规则、过滤URL中可变参数优化缓存命中率，具体操作请参见[优化CDN缓存命中率](products/cdn/documents/use-cases/increase-the-cache-hit-ratios-of-alibaba-cloud-cdn.md)。首先参见以下操作分析具体原因：
...
- 检查是否开启强制Range回源，详情请参见[配置Range回源](products/cdn/documents/user-guide/object-chunking.md)。此功能开启后，所有回源请求将按照设定的Range size切片回源。 Range的功能也有两面性，对于源站是大文件的（超过50M） 场景，Range分片可以有效的降低大文件下载时遇到的网络拥塞，以及源站为窄带的情况。但如果源站的文件平均size相对较小，比如在10M以内的文件，开启强制Range回源没有优化效果，反而降低回源的效率，扩大回源，使命中率降低。
...
- [CDN缓存命中率下降的因素](products/cdn/documents/cache-hit-ratio-of-an-alibaba-cloud-cdn-pop-decreases.md)

- [URL的传递参数为变量导致CDN缓存命中率低](products/cdn/documents/the-passing-parameter-of-the-url-is-a-variable-resulting-in-a-low-alibaba-cloud-content-delivery-network-cache-hit-rate.md)

- [CDN缓存规则](products/cdn/documents/user-guide/what-is-caching.md)
