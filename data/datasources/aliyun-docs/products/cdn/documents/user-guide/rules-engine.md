...
阿里云CDN产品控制台提供了配置缓存过期时间、回源参数改写等基础功能，满足大部分通用需求。但对于一些特殊需求，如将包含路径/example的请求回源到指定源站地址，则需要结合规则引擎实现自定义配置。此外，阿里云CDN还提供了[边缘脚本](products/cdn/documents/user-guide/edgescript-overview.md)功能，支持高度灵活的用户定制需求，配置类似第三方 CDN 服务商（如 Fastly、CloudFront）的精细化策略。
...
规则引擎支持基础的 AND/OR 逻辑组合，适用于大部分条件过滤场景。但以下场景建议使用[边缘脚本（EdgeScript）](products/cdn/documents/user-guide/edgescript-overview.md)替代：
...
User-Agent 匹配值最多支持配置 32 个，超过该数量的配置不生效。若需匹配大量 User-Agent，建议使用通配符（*）合并相似的 UA 值（例如*Chrome*可覆盖所有 Chrome 浏览器版本），或改用[边缘脚本](products/cdn/documents/user-guide/edgescript-overview.md)实现更灵活的匹配逻辑。
...
| 参数名称 | 域名配置功能函数 [condition](products/cdn/documents/developer-reference/parameters-for-configuring-features-for-domain-names.md) 中对应的配置参数 | 参数说明 | 是否必填 |
| --- | --- | --- | --- |
...
| 匹配类型名称 | 域名配置功能函数 [condition](products/cdn/documents/developer-reference/parameters-for-configuring-features-for-domain-names.md) 中对应的配置参数 | 匹配类型含义 | 匹配对象 | 匹配运算符 | 匹配值 | 大小写敏感 | 对应 nginx/tengine |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
| 客户端 IP | clientip | 客户端的 IP。 支持 IPv4（例如 1.1.X.X ）、IPv6（例如 240e:95c:3004:2:3:0:0:XXX ）、支持网段（例如 20.209.XXX.XXX/31 ）。 | 建联 IP XFF IP 说明 建联 IP、XFF IP 详细说明，请参见 [IP](products/cdn/documents/user-guide/rules-engine.md) [地址校验模式](products/cdn/documents/user-guide/rules-engine.md) 。 | 包含其中任意一个 不包含其中任意一个 | 支持填写 IPv6 格式 IP，例如：240e:XXX:3004:2:3:0:0:3f7，支持网段方式填写，例如：120.209.XXX.XXX/31，支持输入多个值。 | 不涉及 | $remote_addr |
| 客户端 IP 版本 | clientipVer | IPv4 或 IPv6。 | 建联 IP XFF IP 说明 建联 IP、XFF IP 详细说明，请参见 [IP](products/cdn/documents/user-guide/rules-engine.md) [地址校验模式](products/cdn/documents/user-guide/rules-engine.md) 。 | 等于 不等于 | v4 v6 | 不涉及 | - |
| 用户网络运营商 | geolocation | 客户端 IP 归属的运营商。 | 建联 IP XFF IP 说明 建联 IP、XFF IP 详细说明，请参见 [IP](products/cdn/documents/user-guide/rules-engine.md) [地址校验模式](products/cdn/documents/user-guide/rules-engine.md) 。 | 包含其中任意一个 不包含其中任意一个 | 可以通过下拉列表来选择，可以输入字符来过滤选项，支持输入 ID 或名称来模糊匹配查询，支持输入多个值。 | 不涉及 | $ip_isp_id |
| 用户 IP 地理位置 | geolocation | 客户端 IP 所处的地理位置。 | 建联 IP XFF IP 说明 建联 IP、XFF IP 详细说明，请参见 [IP](products/cdn/documents/user-guide/rules-engine.md) [地址校验模式](products/cdn/documents/user-guide/rules-engine.md) 。 | 包含其中任意一个 不包含其中任意一个 | 可以通过下拉列表来选择，可以输入字符来过滤选项，支持输入 ID 或名称来模糊匹配查询，支持输入多个值。 | 不涉及 | $ip_country_id |
...
| 名称 | 域名配置功能函数 [condition](products/cdn/documents/developer-reference/parameters-for-configuring-features-for-domain-names.md) 中对应的配置参数 | 含义 |
| --- | --- | --- |
...
| 功能分类 | 功能名称 |
| --- | --- |
| 基本配置 | [条件源站](products/cdn/documents/user-guide/configure-a-conditional-origin.md) |
| 缓存配置 | [缓存过期时间](products/cdn/documents/user-guide/configure-the-cdn-cache-expiration-time.md) |
| [修改出站响应头](products/cdn/documents/user-guide/create-a-custom-http-response-header.md) |  |
| [访问](products/cdn/documents/user-guide/create-an-access-url-rewrite-rule.md) [URL](products/cdn/documents/user-guide/create-an-access-url-rewrite-rule.md) [改写规则](products/cdn/documents/user-guide/create-an-access-url-rewrite-rule.md) |  |
| [自定义](products/cdn/documents/user-guide/create-custom-cache-keys.md) [Cache Key](products/cdn/documents/user-guide/create-custom-cache-keys.md) |  |
| 回源配置 | [修改出站请求头](products/cdn/documents/user-guide/configure-custom-request-headers.md) |
| [修改入站响应头](products/cdn/documents/user-guide/rewrite-http-response-headers.md) |  |
| [重写回源参数](products/cdn/documents/user-guide/rewrite-url-parameters-in-back-to-origin-requests.md) |  |
| [指定源站回源](products/cdn/documents/user-guide/specify-an-origin-host-for-each-origin.md) [HOST](products/cdn/documents/user-guide/specify-an-origin-host-for-each-origin.md) |  |
...
| [配置](products/cdn/documents/user-guide/configure-url-signing.md) [URL](products/cdn/documents/user-guide/configure-url-signing.md) [鉴权](products/cdn/documents/user-guide/configure-url-signing.md) |  |
| [配置](products/cdn/documents/user-guide/configure-an-ip-blacklist-or-whitelist.md) [IP](products/cdn/documents/user-guide/configure-an-ip-blacklist-or-whitelist.md) [黑/白名单](products/cdn/documents/user-guide/configure-an-ip-blacklist-or-whitelist.md) |  |
| [配置](products/cdn/documents/user-guide/configure-a-user-agent-blacklist-or-whitelist.md) [UA](products/cdn/documents/user-guide/configure-a-user-agent-blacklist-or-whitelist.md) [黑白名单](products/cdn/documents/user-guide/configure-a-user-agent-blacklist-or-whitelist.md) |  |
| 性能优化 | [忽略参数](products/cdn/documents/user-guide/ignore-parameters.md) |
| 视频相关 | [配置](products/cdn/documents/user-guide/object-chunking.md) [Range](products/cdn/documents/user-guide/object-chunking.md) [回源](products/cdn/documents/user-guide/object-chunking.md) |
| 流量限制 | [单请求限速](products/cdn/documents/user-guide/configuration-order-request-speed-limit.md) |
...
[上一篇：示例：通过CLI工具使用边缘脚本](products/cdn/documents/user-guide/use-the-es-cli-to-manage-scripts.md)[下一篇：安全配置](products/cdn/documents/user-guide/security-configuration.md)
