...
| 功能 | 说明 |
| --- | --- |
| [下载离线日志](products/cdn/documents/user-guide/offline-logs-quick-start.md) | 您可以查询指定时间、域名下日志，并下载保存。 |
| [通过函数计算转存离线日志](products/cdn/documents/user-guide/use-function-compute-to-deliver-logs.md) | 如果您需要将日志保存更长时间，则可以将日志转存至 OSS，方便您根据实际情况对日志进行保存和分析。 |
| [配置实时日志推送](products/cdn/documents/user-guide/configure-real-time-log-delivery.md) | 您可以通过实时日志推送功能，将 CDN 日志实时推送至日志服务，并进行日志分析，便于快速发现和定位问题。 |
...
对象存储OSS是一款海量、安全、低成本、高可靠的云存储服务，用于存储和管理各种类型的文件。更多信息，请参见[什么是对象存储](products/oss/documents/user-guide/what-is-oss.md)[OSS](products/oss/documents/user-guide/what-is-oss.md)。

- 

日志服务

日志服务提供实时的数据收集、存储和查询功能。更多信息，请参见[什么是日志服务](products/sls/documents/what-is-log-service.md)。

[上一篇：离线日志](products/cdn/documents/user-guide/offline-logs-1.md)[下一篇：快速入门](products/cdn/documents/user-guide/offline-logs-quick-start.md)
