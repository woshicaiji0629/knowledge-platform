...
调用[BatchSetCdnDomainConfig](products/cdn/documents/developer-reference/api-cdn-2018-05-10-batchsetcdndomainconfig.md)接口，可以批量配置域名。更多功能参数的配置方法，敬请参考[域名配置功能函数](products/cdn/documents/developer-reference/parameters-for-configuring-features-for-domain-names.md)。

## 使规则变更立即生效

配置变更或新增规则，仅对新缓存的资源生效。变更前已缓存的资源，将继续沿用旧的缓存策略直至过期。

要使新规则立即对全网生效，必须手动清理已有缓存。如果是规则变更，通过[刷新和预热资源](products/cdn/documents/user-guide/refresh-and-prefetch-resources.md)功能执行刷新操作；如果是新增规则，通过[预热资源](products/cdn/documents/user-guide/refresh-and-prefetch-resources.md)功能执行预热操作。
...
游戏行业及小文件场景：对于游戏行业的小文件资源（如配置文件、素材包），若更新频率较低（例如每周或每两周更新一次），建议将缓存过期时间设置为 15 天或与资源实际更新周期保持一致，以平衡更新速度与加速效果。过短的缓存时间会导致频繁回源，影响加速效果；过长则可能导致用户获取到旧版本资源。建议结合[刷新预热](products/cdn/documents/user-guide/refresh-and-prefetch-resources.md)功能，在版本更新时主动刷新缓存。
...
[缓存相关常见问题](products/cdn/documents/user-guide/cache-related-faq.md)

[上一篇：什么是缓存](products/cdn/documents/user-guide/what-is-caching.md)[下一篇：配置状态码过期时间](products/cdn/documents/user-guide/create-a-cache-rule-for-http-status-codes.md)
