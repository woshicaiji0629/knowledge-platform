...
阿里云账号拥有所有API的访问权限，风险很高。强烈建议您创建并使用RAM用户进行API访问或日常运维。请根据业务的实际情况按需分配权限后进行接口调用。RAM用户需具备操作CDN资源的权限。具体操作，请参见[CDN](products/cdn/documents/security-and-compliance/cdn.md)[系统权限策略参考](products/cdn/documents/security-and-compliance/cdn.md)。
...
[上一篇：CLI集成示例](products/cdn/documents/developer-reference/cli-integration-example.md)[下一篇：附录](products/cdn/documents/developer-reference/apiappendix.md)
