...
您需要创建一个RAM用户并授予相应操作权限。具体操作，请参见[创建](products/ram/documents/create-a-ram-user-1.md)[RAM](products/ram/documents/create-a-ram-user-1.md)[用户](products/ram/documents/create-a-ram-user-1.md)及[管理](products/ram/documents/user-guide/grant-permissions-to-the-ram-user.md)[RAM](products/ram/documents/user-guide/grant-permissions-to-the-ram-user.md)[用户的权限](products/ram/documents/user-guide/grant-permissions-to-the-ram-user.md)。

重要

本示例需要您为RAM用户授予AliyunCDNReadOnlyAccess权限策略。您也可以按需选择AliyunCDNFullAccess权限（具有查询、修改CDN域名的完全控制权限）或进行自定义策略，更多信息请参见[CDN](products/cdn/documents/security-and-compliance/custom-policies-for-dcdn.md)[自定义权限策略参考](products/cdn/documents/security-and-compliance/custom-policies-for-dcdn.md)。

- 

创建RAM用户并授权后，您需要创建RAM用户对应的AccessKey，并记录AccessKey ID和AccessKey Secret，以便后续配置身份凭证使用。具体操作，请参见[创建](products/ram/documents/user-guide/create-an-accesskey-pair.md)[AccessKey](products/ram/documents/user-guide/create-an-accesskey-pair.md)。
...
使用RAM用户的AccessKey配置AK类型凭证，配置文件命名为AkProfile。具体操作，请参见[服务接入点](products/cdn/documents/developer-reference/api-cdn-2018-05-10-endpoint.md)。
...
以下示例将为您展示如何使用阿里云CLI调用CDN中的DescribeUserDomains命令，查询用户名下所有的域名与状态。DescribeUserDomains命令的详细介绍，请参见[DescribeUserDomains](products/cdn/documents/developer-reference/api-cdn-2018-05-10-describeuserdomains.md)。
...
[上一篇：Terraform配置常见问题](products/cdn/documents/developer-reference/terraform-configuration-faq.md)[下一篇：资源编排ROS集成示例](products/cdn/documents/developer-reference/resource-orchestration-ros-integration-example.md)
