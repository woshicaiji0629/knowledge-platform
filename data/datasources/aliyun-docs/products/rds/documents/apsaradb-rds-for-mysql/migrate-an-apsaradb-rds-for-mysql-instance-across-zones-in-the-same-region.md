...
实例规格：不能是[历史规格](products/rds/documents/apsaradb-rds-for-mysql/primary-apsaradb-rds-for-mysql-instance-types.md)。如需变更实例规格，请参见[变更配置](products/rds/documents/apsaradb-rds-for-mysql/change-the-specifications-of-an-apsaradb-rds-for-mysql-instance.md)。
...
如果为云盘实例，内核小版本不可低于20201031。如何升级内核小版本，请参见[升级内核小版本](products/rds/documents/apsaradb-rds-for-mysql/update-the-minor-engine-version-of-an-apsaradb-rds-for-mysql-instance.md)。
...
共享代理功能已于2021年04月01日停止更新维护，若您仍在使用共享代理，建议您[升级共享代理为独享型代理](products/rds/documents/apsaradb-rds-for-mysql/upgrade-the-database-proxy-of-an-apsaradb-rds-for-mysql-instance-from-a-shared-proxy-to-a-dedicated-proxy.md)。
...
[PostgreSQL](products/rds/documents/apsaradb-rds-for-postgresql/migrate-an-apsaradb-rds-for-postgresql-instance-across-zones-in-the-same-region.md)[迁移可用区](products/rds/documents/apsaradb-rds-for-postgresql/migrate-an-apsaradb-rds-for-postgresql-instance-across-zones-in-the-same-region.md)

- 

[SQL Server](products/rds/documents/apsaradb-rds-for-sql-server/migrate-an-apsaradb-rds-for-sql-server-instance-across-zones.md)[迁移可用区](products/rds/documents/apsaradb-rds-for-sql-server/migrate-an-apsaradb-rds-for-sql-server-instance-across-zones.md)
...
仅支持在同地域下的可用区之间进行迁移。如需跨地域迁移可用区，可以在目标地域和可用区[购买新实例](products/rds/documents/apsaradb-rds-for-mysql/create-an-apsaradb-rds-for-mysql-instance-1.md)，使用DTS[将原实例数据迁移至新实例](products/rds/documents/apsaradb-rds-for-mysql/migrate-data-between-apsaradb-rds-for-mysql-instances.md)，检查无误后[释放原实例](products/rds/documents/apsaradb-rds-for-mysql/release-or-unsubscribe-from-an-instance.md)。

- 

主节点切换：迁移可用区期间可能会导致主节点切换，造成主节点连接地址、代理连接地址短时间不可用，请确保应用具有自动重连机制。若无自动重连机制，请手动进行应用与数据库的重连。具体影响请参见[实例切换的影响](products/rds/documents/apsaradb-rds-for-mysql/untitled-document-1701914031929.md)。主节点切换的场景如下：
...
迁移可用区时，不支持只更改交换机。若您需要修改实例的交换机，请参见[切换专有网络](products/rds/documents/apsaradb-rds-for-mysql/change-the-vpc-and-vswitch-for-an-apsaradb-rds-for-mysql-instance.md)[VPC](products/rds/documents/apsaradb-rds-for-mysql/change-the-vpc-and-vswitch-for-an-apsaradb-rds-for-mysql-instance.md)[和虚拟交换机](products/rds/documents/apsaradb-rds-for-mysql/change-the-vpc-and-vswitch-for-an-apsaradb-rds-for-mysql-instance.md)。

- 

当实例存储类型为高性能云盘并且开启了[Buffer Pool Extension（BPE）功能](products/rds/documents/apsaradb-rds-for-mysql/buffer-pool-extension-bpe.md)时，无法迁移至不支持Buffer Pool Extension（BPE）的可用区。Buffer Pool Extension（BPE）支持的地域及可用区请参见[适用范围](products/rds/documents/apsaradb-rds-for-mysql/buffer-pool-extension-bpe.md)。
...
在迁移至目标可用区时，必须确保该可用区内已配置专有网络（VPC）和至少一个交换机（VSwitch）。若目标可用区内不存在交换机，则无法进行选择和迁移，请先[创建交换机](products/vpc/documents/user-guide/create-and-manage-vswitch.md)。
...
实例切换的影响请参见[实例切换的影响](products/rds/documents/apsaradb-rds-for-mysql/untitled-document-1701914031929.md)。
...
| API | 描述 |
| --- | --- |
| [迁移可用区](products/rds/documents/api-migrate-an-instance-across-zones.md) | 迁移 RDS 实例可用区。 |
...
迁移过程中会出现实例切换，请确保应用具有自动重连机制。若应用没有自动重连机制，需手动重连。实例切换的影响请参见[实例切换的影响](products/rds/documents/apsaradb-rds-for-mysql/untitled-document-1701914031929.md)。
...
[上一篇：云盘存储空间缩容](products/rds/documents/apsaradb-rds-for-mysql/reduce-the-storage-capacity-of-an-apsaradb-rds-for-mysql-instance-that-uses-essds.md)[下一篇：设置可维护时间段](products/rds/documents/apsaradb-rds-for-mysql/set-the-maintenance-window-of-an-apsaradb-rds-for-mysql-instance.md)
