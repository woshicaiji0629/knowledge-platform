...
RDS PostgreSQL的大版本升级功能可以在升级后保留原实例的设置，例如，白名单、参数设置、插件（新版本不支持的插件和参数除外）。此外，[加密](products/rds/documents/overview.md)[RDS PostgreSQL](products/rds/documents/overview.md)[实例](products/rds/documents/overview.md)升级大版本后仍为加密实例，且加密密钥将保持不变。
...
| 升级方案 | 本地升级 | 蓝绿部署 | 零停机 | [使用](products/rds/documents/apsaradb-rds-for-postgresql/use-dts-to-migrate-data-between-apsaradb-rds-for-postgresql-instances.md) [DTS](products/rds/documents/apsaradb-rds-for-postgresql/use-dts-to-migrate-data-between-apsaradb-rds-for-postgresql-instances.md) [迁移数据](products/rds/documents/apsaradb-rds-for-postgresql/use-dts-to-migrate-data-between-apsaradb-rds-for-postgresql-instances.md) |  |
| --- | --- | --- | --- | --- | --- |
| 割接 | 不割接 |  |  |  |  |
...
对于本地升级模式，如果在升级过程中实例未达推荐规格，系统将自动尝试使用推荐规格进行升级。这将导致分钟级的只读状态，并额外出现一次秒级闪断。建议升级前，处理[大版本升级检查报告](products/rds/documents/apsaradb-rds-for-postgresql/introduction-to-the-check-report-of-a-major-engine-version-upgrade-for-an-apsaradb-rds-for-postgresql-instance.md)中关于实例规格的告警。

## 大版本升级

### 方式一：[通过本地升级模式升级大版本](products/rds/documents/apsaradb-rds-for-postgresql/upgrade-major-engine-version-in-local-upgrade-mode.md)

### 方式二：[通过蓝绿部署模式升级大版本](products/rds/documents/apsaradb-rds-for-postgresql/upgrade-major-engine-version-in-blue-green-deployment-mode.md)

### 方式三：[通过零停机模式升级大版本](products/rds/documents/apsaradb-rds-for-postgresql/upgrade-major-engine-version-in-zero-downtime-mode.md)
...
[创建新实例](products/rds/documents/apsaradb-rds-for-postgresql/create-an-apsaradb-rds-for-postgresql-instance-1.md)。

- 

[数据迁移至新实例](products/rds/documents/apsaradb-rds-for-mysql/migrate-data-between-apsaradb-rds-for-mysql-instances.md)。

- 

[释放原实例](products/rds/documents/apsaradb-rds-for-postgresql/release-or-unsubscribe-from-an-apsaradb-rds-for-postgresql-instance.md)

[上一篇：升级版本](products/rds/documents/apsaradb-rds-for-postgresql/version-upgrade-2.md)[下一篇：通过本地升级模式升级大版本](products/rds/documents/apsaradb-rds-for-postgresql/upgrade-major-engine-version-in-local-upgrade-mode.md)
