...
| --- | --- | --- |
| [构建智能客服](products/model-studio/documents/single-agent-application.md) 通过可视化工具快速搭建 AI 助手，处理客户咨询。 | [编排流程](products/model-studio/documents/workflow-application.md) 可视化流程编排，无代码基础的业务人员也能独立完成工作流设计。 | [微调模型](products/model-studio/documents/model-training-overview.md) 可视化微调，无需编写训练代码，即可完成模型定制。 |
...
百炼提供开箱即用的模型服务，无需自行部署或运维，直接调用自研千问（Qwen）全系列模型，以及 DeepSeek、Kimi、GLM 等第三方大模型。详情请参见[选择模型](products/model-studio/documents/models.md)。
...
[模型调优](products/model-studio/documents/model-training-overview.md)：支持有监督微调（SFT）、继续预训练（CPT）和直接偏好优化（DPO），满足特定业务需求。

- 

[模型部署](products/model-studio/documents/model-deployment-introduction.md)：将预置模型或调优后的自定义模型部署为资源专享的推理服务，满足高并发、低延迟等性能要求。支持按时长、包月、按 Token 量等多种计费方式。

- 

[模型评测](products/model-studio/documents/model-evaluation-overview.md)：提供人工评测、自动评测和基线评测，支持快速对比不同模型表现，验证调优效果，提前发现潜在调用风险。

## 应用构建

- 

应用类型：提供可视化和高代码两种开发模式。可视化模式可快速创建[智能体应用](products/model-studio/documents/single-agent-application.md)和[工作流应用](products/model-studio/documents/workflow-application.md)；[高代码应用](products/model-studio/documents/rich-code-application.md)则支持将 Python 项目部署为后端服务，具备自动化运维、可观测、日志服务等能力。

- 

功能拓展：通过[知识库（RAG）](products/model-studio/documents/rag-knowledge-base.md)接入私有数据和专业领域知识；通过[插件](products/model-studio/documents/plug-in-overview.md)和[模型上下文协议（MCP）](products/model-studio/documents/mcp-introduction.md)调用外部服务。

- 

分享与发布：支持将应用发布至网页、钉钉机器人、微信公众号及音视频互动智能体等多种平台，详见[应用分享](products/model-studio/documents/share-an-application.md)。

## 产品计费

开通百炼无需费用，调用、微调、部署模型时产生相应费用，详情请参见[计费项](products/model-studio/documents/billing-for-model-studio.md)。
...
已认证用户免费额度用完后自动转为按量付费，如需避免意外扣费，可开启[免费额度用完即停](products/model-studio/documents/new-free-quota.md)功能，额度耗尽时服务自动停止。

详见[新人免费额度](products/model-studio/documents/new-free-quota.md)。
...
调用统计：模型调用完约一小时后，前往[阿里云百炼控制台](https://bailian.console.aliyun.com/cn-beijing?spm=5176.29619931.J__Z58Z6CX7MY__Ll8p1ZOR.1.7dd7521cmX1pAh&tab=model#/model-market)，在页面右上角选择目标地域，进入[模型监控](https://bailian.console.aliyun.com/?tab=model#/model-telemetry)页面并设置查询条件，点击目标模型操作列的监控，即可查看调用量、Token 消耗、成功率等统计数据。详情请参见[模型监控](products/model-studio/documents/model-telemetry.md)。

- 

Coding Plan 用量：如已订阅 Coding Plan（AI 编码套餐），可在[Coding Plan 页面](https://bailian.console.aliyun.com/cn-beijing/?tab=model#/efm/coding_plan)查看当前套餐的请求消耗情况。Coding Plan 采用固定月费，提供月度请求额度，支持在 AI 编码工具中使用，详情请参见[Coding Plan](products/model-studio/documents/coding-plan.md)[概述](products/model-studio/documents/coding-plan.md)。
...
发起第一个API请求：[首次调用千问](products/model-studio/documents/first-api-call-to-qwen.md)[API](products/model-studio/documents/first-api-call-to-qwen.md)

- 

构建第一个大模型应用：[0](products/model-studio/documents/build-knowledge-base-qa-assistant-without-coding.md)[代码构建问答应用](products/model-studio/documents/build-knowledge-base-qa-assistant-without-coding.md)

## 常见问题

Q：我的数据安全吗？阿里云百炼会用我的数据进行训练吗？

A：不会。阿里云严格保护数据隐私，不会将您的数据用于模型训练。构建应用或训练模型时传输的所有数据均经过加密处理。详情请参见[合规资质与隐私说明](products/model-studio/documents/privacy-notice.md)。
...
建议选择邻近地域以降低网络延迟。各地域的接入点（Endpoint/Base URL）不同，API Key 不通用，支持的模型、平台功能与价格也有所差异，详情请参见[选择模型](products/model-studio/documents/models.md)。
...
开启"[免费额度用完即停](products/model-studio/documents/new-free-quota.md)"（仅限新用户且在免费额度有效期内）：在模型详情页开启此开关，免费额度耗尽后服务自动停止，不会转为付费。仅适用于华北2（北京）地域（[中国内地服务部署范围](products/model-studio/documents/regions.md)），且须在免费额度有效期内。
...
订阅 Coding Plan（AI 编码套餐）：固定月费，提供月度请求额度，无按量扣费风险。注意需使用 Coding Plan 专属的 Base URL和API Key 进行调用，否则模型调用将按量付费。详情请参见[Coding Plan](products/model-studio/documents/coding-plan.md)[概述](products/model-studio/documents/coding-plan.md)。
...
通过API调用：请参见[首次调用千问](products/model-studio/documents/first-api-call-to-qwen.md)[API](products/model-studio/documents/first-api-call-to-qwen.md)。

- 

通过开发工具（如 Claude Code）调用：请参考[接入客户端/开发工具](products/model-studio/documents/use-chat-client-or-development-tool.md)。

- 

通过可视化界面构建大模型应用：请参考[智能体应用](products/model-studio/documents/single-agent-application.md)或[工作流应用](products/model-studio/documents/workflow-application.md)。

[上一篇：开始使用](products/model-studio/documents/get-started-with-models.md)[下一篇：首次调用千问API](products/model-studio/documents/first-api-call-to-qwen.md)
