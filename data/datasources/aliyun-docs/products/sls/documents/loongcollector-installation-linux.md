...
若您无可用Project，请参考此处步骤创建一个基础Project，如需详细了解创建配置请参见[管理](products/sls/documents/manage-a-project.md)[Project](products/sls/documents/manage-a-project.md)。
...
若您无可用LogStore，请参考此处步骤创建一个基础LogStore，如需详细了解创建配置请参见[管理](products/sls/documents/manage-a-logstore.md)[LogStore](products/sls/documents/manage-a-logstore.md)。
...
当安装方式选择：[同账号同地域](products/sls/documents/loongcollector-installation-linux.md)、[不同账号同地域](products/sls/documents/loongcollector-installation-linux.md)时，数据传输方式为内网传输，替换${project名称}为Project名称，${域名信息}为私网域名。

- 

当安装方式选择：[同账号不同地域](products/sls/documents/loongcollector-installation-linux.md)、[其他云/自建服务器](products/sls/documents/loongcollector-installation-linux.md)时，数据传输方式为公网传输，替换${project名称}为Project名称，${域名信息}为公网域名。
...
仅当服务器为阿里云ECS，且ECS与Project属于同一个阿里云账号，所属[地域](products/sls/documents/loongcollector-installation-linux.md)也相同时，日志服务可一键在ECS中安装LoongCollector，借助OOS编排能力，无需登录ECS手动执行安装步骤。

一键安装能力已集成到日志服务的接入模板中，日志服务提供了正则、单行、多行等多种文本日志接入模板，各模板之间仅[处理插件](products/sls/documents/processing-plug-ins.md)不同；模板内支持添加、删除处理插件。请根据采集日志的特点选择模板，或任意选择文本日志模板后再根据日志特点进行处理插件配置。
...
至此一键安装完成。单击下一步，将进行[采集配置](products/sls/documents/host-text-log-collection-auto-install.md)。

### 同账号不同地域

当服务器为阿里云ECS，且ECS与Project属于同一个阿里云账号，但不属于同一个[地域](products/sls/documents/loongcollector-installation-linux.md)，此时需要手动下载安装包，并在安装命令中使用[公网传输方式或传输加速](products/sls/documents/loongcollector-installation-linux.md)。

具体操作如下：

- 

下载安装包：在服务器上执行下载命令，示例代码中${region_id}可使用cn-hangzhou替换，若想加快安装包下载速度，请参考[RegionID](products/sls/documents/loongcollector-installation-linux.md)替换${region_id}为ECS所属地域。
...
选择传输方式并执行安装命令：替换${region_id}为Project所属地域的[RegionID](products/sls/documents/loongcollector-installation-linux.md)。
...
需要先打开Project的[日志跨域传输加速](products/sls/documents/manage-a-project.md)功能，再执行安装命令。chmod +x loongcollector.sh; ./loongcollector.sh install ${region_id}-acceleration
...
安装完成后若需要采集日志还需进行[采集配置](products/sls/documents/host-text-log-collection-auto-install.md)。

### 不同账号同地域

当服务器为阿里云ECS，且ECS与Project属于同一个[地域](products/sls/documents/loongcollector-installation-linux.md)，但不属于同一个阿里云账号时，需要手动下载安装包，并在安装命令中使用[内网传输方式](products/sls/documents/loongcollector-installation-linux.md)，且需要配置用户ID。
...
执行安装命令：${region_id}需替换为Project所属地域的[RegionID](products/sls/documents/loongcollector-installation-linux.md)。
...
当服务器是其他云服务器或自建服务器时（若服务器为阿里云ECS，但ECS与Project不属于同一个阿里云账号，也不在同一个[地域](products/sls/documents/loongcollector-installation-linux.md)时，也可视为自建服务器），需要手动下载安装包，并在安装命令中使用[公网传输方式或传输加速](products/sls/documents/loongcollector-installation-linux.md)，且需要配置用户ID。

说明

服务器使用限制请参考[支持的](products/sls/documents/loongcollector-installation-linux.md)[Linux](products/sls/documents/loongcollector-installation-linux.md)[系统版本](products/sls/documents/loongcollector-installation-linux.md)。
...
下载与安装命令请参考上述安装场景选择，如何使用ECS云助手请参考[创建/执行命令](products/ecs/documents/user-guide/use-the-immediate-execution-feature.md)。
...
在服务器上执行下载命令获取最新安装包，示例代码中${region_id}可使用cn-hangzhou替换，若想加快安装包下载速度，请参考[地域](products/sls/documents/loongcollector-installation-linux.md)替换${region_id}为ECS所属地域。
...
在服务器上执行下载命令获取安装包，示例代码中${region_id}可使用cn-hangzhou替换，若想加快安装包下载速度，请参考[地域](products/sls/documents/loongcollector-installation-linux.md)替换${region_id}为ECS所属地域。
...
示例代码中${region_id}可使用cn-hangzhou替换，若想加快执行速度，请参考[地域](products/sls/documents/loongcollector-installation-linux.md)替换${region_id}为ECS所属地域。
...
在需要安装LoongCollector的服务器上执行uname -m查看系统架构后，在可以访问公网的服务器上选择对应下载命令执行：${region_id}需替换为Project所属地域的[RegionID](products/sls/documents/loongcollector-installation-linux.md)。
...
将下载的安装脚本和安装包拷贝至需要安装LoongCollector的服务器上，执行如下命令：${region_id}需替换为Project所属地域的[RegionID](products/sls/documents/loongcollector-installation-linux.md)。
...
由于服务器无法访问公网，你还需要通过[配置代理](products/sls/documents/how-do-i-collect-logs-from-servers-in-a-corporate-intranet.md)的方式与公网建立连接。

### 服务器无法连接外部网络时如何升级

- 

在需要升级LoongCollector的服务器上执行uname -m查看系统架构后，在可以访问公网的服务器上选择对应命令执行：${region_id}需替换为Project所属地域的[地域](products/sls/documents/loongcollector-installation-linux.md)。
...
自动安装仅支持ECS与Project同账号同地域情况，若不满足请选择其他安装方式，若满足请检查[前提条件](products/sls/documents/loongcollector-installation-linux.md)中权限与网络要求是否满足。
...
若流程选择正确但心跳为FAIL，请查看安装LoongCollector的服务器上/usr/local/ilogtail/ilogtail_config.json文件中region信息是否与日志服务Project地域的[RegionID](products/sls/documents/loongcollector-installation-linux.md)一致。
...
若心跳仍然为FAIL，请检查是否满足[前提条件](products/sls/documents/loongcollector-installation-linux.md)中的网络要求。
...
[上一篇：Linux 安装与管理](products/sls/documents/loongcollector-installation-and-management-linux.md)[下一篇：配置管理](products/sls/documents/loongcollector-management-linux.md)
