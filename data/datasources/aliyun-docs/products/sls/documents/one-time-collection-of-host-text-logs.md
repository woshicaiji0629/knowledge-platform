...
[准备工作](products/sls/documents/one-time-collection-of-host-text-logs.md)：创建Project和LogStore，Project是资源管理单元，用于隔离不同业务日志，而LogStore用于存储日志。

- 

[配置机器组（安装](products/sls/documents/one-time-collection-of-host-text-logs.md)[LoongCollector）](products/sls/documents/one-time-collection-of-host-text-logs.md)：根据服务器类型，安装LoongCollector，并将其加入到机器组。使用机器组统一管理采集节点，对服务器进行配置分发与状态管理。

- 

[创建并配置一次性文件采集规则](products/sls/documents/one-time-collection-of-host-text-logs.md)：

- 

[全局与输入配置](products/sls/documents/one-time-collection-of-host-text-logs.md)：定义采集配置的名称及日志采集的来源和范围。

- 

[日志处理与结构化](products/sls/documents/one-time-collection-of-host-text-logs.md)：根据日志格式进行处理配置。
...
[日志过滤](products/sls/documents/one-time-collection-of-host-text-logs.md)：通过配置采集黑名单和内容过滤规则，筛选有效日志内容，减少冗余数据的传输与存储。

- 

[日志分类](products/sls/documents/one-time-collection-of-host-text-logs.md)：通过配置日志主题（Topic）灵活区分不同业务、服务器、路径来源的日志。

- 

[查询与分析配置](products/sls/documents/one-time-collection-of-host-text-logs.md)：系统默认开启全文索引，支持关键词搜索。建议启用字段索引，以便对结构化字段进行精确查询和分析，提升检索效率。

- 

[验证采集结果](products/sls/documents/one-time-collection-of-host-text-logs.md)：完成配置后，验证日志是否成功采集，如遇采集无数据、心跳失败或解析错误等问题，参考[常见问题](products/sls/documents/one-time-collection-of-host-text-logs.md)。

## 准备工作

在采集日志前，需规划并创建用于管理与存储日志的Project和LogStore。若已有可用资源，可跳过此步骤，直接进入[配置机器组（安装](products/sls/documents/one-time-collection-of-host-text-logs.md)[LoongCollector）](products/sls/documents/one-time-collection-of-host-text-logs.md)。
...
在完成[准备工作](products/sls/documents/one-time-collection-of-host-text-logs.md)后，为不同类型的服务器安装LoongCollector并将其加入机器组。
...
如需将已安装LoongCollector的服务器加入已有机器组，请参考常见问题[如何将服务器加入到已有机器组？](products/sls/documents/one-time-collection-of-host-text-logs.md)

- 

检查心跳状态：单击下一步，如页面出现机器组心跳情况。查看心跳状态，若为OK表示机器组连接正常，单击下一步，进入Logtail配置页面。

若为FAIL，可能是初次建立心跳需要花费一些时间，请等待两分钟左右，再刷新心跳状态。若刷新后仍为FAIL，请参考[机器组心跳连接为](products/sls/documents/one-time-collection-of-host-text-logs.md)[fail](products/sls/documents/one-time-collection-of-host-text-logs.md)进一步排查。

## 步骤二：创建并配置一次性文件采集规则

完成[LoongCollector](products/sls/documents/one-time-collection-of-host-text-logs.md)[安装和机器组配置](products/sls/documents/one-time-collection-of-host-text-logs.md)后，进入Logtail配置页面，定义日志采集和处理规则。
...
开启：更新采集配置时强制重新执行采集任务，确保所有数据的处理与上报符合最新改动的要求。注意：更新前已经采集的部分数据不会消失，如需清理请参考[日志服务软删除](products/sls/documents/soft-delete.md)）。
...
添加解析插件：单击添加处理插件，根据实际格式配置[正则解析、分隔符解析、JSON 解析等插件](products/sls/documents/one-time-collection-of-host-text-logs.md)。此处以采集NGINX日志为例，选择原生处理插件>NGINX模式解析。
...
[旧版历史文件采集](products/sls/documents/import-historical-logs.md)方式不再推荐使用，请优先选择新版功能进行历史数据导入。相较于旧版手动创建配置文件的采集方式，新版一次性文件采集在配置效率、可靠性和可观测性上均有显著提升。具体对比如下：
...
新服务器已[安装](products/sls/documents/loongcollector-installation-linux.md)[LoongCollector](products/sls/documents/loongcollector-installation-linux.md)。
...
若心跳状态为FAIL，请参考常见问题[机器组心跳连接为](products/sls/documents/one-time-collection-of-host-text-logs.md)[fail](products/sls/documents/one-time-collection-of-host-text-logs.md)进一步排查。
...
其他参数请参考[场景二：结构化日志](products/sls/documents/one-time-collection-of-host-text-logs.md)中的通用配置参数说明。
...
[上一篇：持续采集主机文本日志](products/sls/documents/host-text-log-collection-auto-install.md)[下一篇：Kubernetes集群容器日志采集](products/sls/documents/container-log-collection-in-a-kubernetes-cluster.md)
