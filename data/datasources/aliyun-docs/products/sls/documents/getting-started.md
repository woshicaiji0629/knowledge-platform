...
RAM用户登录：需要主账号[授权](products/sls/documents/create-a-ram-user-and-authorize-the-ram-user-to-access-log-service.md)相应权限策略：
...
在实际生产过程中，可以通过[创建自定义权限策略](products/ram/documents/create-a-custom-policy.md)，对RAM用户实现更精细化的权限管理

准备ECS实例

确保[ECS](products/ecs/documents/user-guide/use-the-ecs-instance-in-the-console.md)[实例](products/ecs/documents/user-guide/use-the-ecs-instance-in-the-console.md)的安全组配置满足出口方向开放80（HTTP）端口和443（HTTPS）端口。

生成模拟日志

- 

[登录](products/ecs/documents/user-guide/connect-to-an-instance-overview.md)[ECS](products/ecs/documents/user-guide/connect-to-an-instance-overview.md)[实例](products/ecs/documents/user-guide/connect-to-an-instance-overview.md)。
...
[其他配置](products/sls/documents/manage-a-project.md)保持默认，单击创建。
...
输入Logstore名称（例如nginx-access-log），[其他配置](products/sls/documents/manage-a-logstore.md)无需修改，单击确定。

默认情况下创建的是[标准型](products/sls/documents/manage-a-logstore.md)，[按写入数据量计费](products/sls/documents/billing-overview.md)的LogStore 。
...
所有图表均可根据业务需求[自定义修改](products/sls/documents/dashboard.md)。
...
示例代码中${region_id}可使用cn-hangzhou替换，若想加快执行速度，请将${region_id}替换为ECS所属[地域](products/sls/documents/loongcollector-installation-linux.md)。
...
理解[Project](products/sls/documents/manage-a-project.md)与[Store](products/sls/documents/manage-sls-store.md)概念。

- 

熟悉[数据采集方式](products/sls/documents/data-collection-overview.md)，根据实际业务场景选择合适的采集方式。

- 

了解[存储资源层级关系说明](products/sls/documents/resource-management-overview.md)并规划资源周期，合理分配Shard数量。

## 常见问题

### 采集日志后，显示时间与原始日志时间不一致怎么办？

默认情况下，日志服务的时间字段（__time__）使用的是日志到达服务器的时间。若要使用日志原文中的时间，需要在采集配置中添加[时间解析插件](products/sls/documents/time-parsing.md)。

### 仅创建Project和LogStore，会产生费用吗？

当您在创建LogStore时，日志服务默认预留Shard资源，因此可能产生活跃Shard租用费用。更多信息，请参见[为什么会产生活跃](products/sls/documents/why-am-i-charged-for-active-shards.md)[Shard](products/sls/documents/why-am-i-charged-for-active-shards.md)[租用费用？](products/sls/documents/why-am-i-charged-for-active-shards.md)

### 日志采集失败，如何排查？

使用Logtail采集日志失败，可能是因为Logtail心跳异常、采集错误、Logtail采集配置错误等原因。如何排查，请参见[Logtail](products/sls/documents/what-do-i-do-if-errors-occur-when-i-use-logtail-to-collect-logs.md)[采集日志失败的排查思路](products/sls/documents/what-do-i-do-if-errors-occur-when-i-use-logtail-to-collect-logs.md)。

### 为什么可以查询到日志，但无法进行分析？

分析日志需要为相关字段配置字段索引并开启统计功能，请检查LogStore的[索引配置](products/sls/documents/create-indexes.md)。

### 如何停止日志服务计费？

日志服务开通后无法关闭，如果不再使用日志服务，可以删除账号下的所有Project即可[停止计费](products/sls/documents/stop-billing.md)。

[上一篇：快速入门](products/sls/documents/toc-of-sls-quick-start.md)[下一篇：快速入门：接入日志服务SDK上报日志并分析](products/sls/documents/use-log-service-sdk-to-collect-and-analyze-logs.md)
