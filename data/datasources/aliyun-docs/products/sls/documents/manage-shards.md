...
例如[Shard](products/sls/documents/shard.md)[范围](products/sls/documents/shard.md)所示，当写入数据时指定MD5的Key值为5F时，则数据将被写入包含5F的Shard1上；当写入数据时指定MD5的Key值为8C时，则数据将被写入包含8C的Shard2上。

### Shard的读写能力

每个Shard提供一定的服务能力，具体请参见[数据读写](products/sls/documents/data-read-and-write.md)。
...
当写入数据量超出当前Shard的[写入服务能力](products/sls/documents/data-read-and-write.md)且持续5分钟以上。
...
如果您在创建LogStore时开启了永久保存，建议您通过删除LogStore的方式删除LogStore中的Shard和数据。更多信息，请参见[停止计费/删除](products/sls/documents/manage-a-logstore.md)[LogStore](products/sls/documents/manage-a-logstore.md)。

## Shard接口

| 操作 | 接口 |
| --- | --- |
| 分裂 Shard | [SplitShard](products/sls/documents/developer-reference/api-sls-2020-12-30-splitshard.md) |
| 合并 Shard | [MergeShard](products/sls/documents/developer-reference/api-sls-2020-12-30-mergeshard.md) |
| 查询 Shard | [ListShards](products/sls/documents/developer-reference/api-sls-2020-12-30-listshards.md) |


[上一篇：管理EventStore](products/sls/documents/manage-an-eventstore.md)[下一篇：资源流控及解决方案](products/sls/documents/expansion-of-resources.md)
