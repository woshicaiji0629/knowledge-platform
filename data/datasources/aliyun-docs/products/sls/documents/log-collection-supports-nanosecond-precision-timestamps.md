...
如果已提前创建，可跳过本步骤，直接进入[配置机器组（安装](products/sls/documents/log-collection-supports-nanosecond-precision-timestamps.md)[LoongCollector）](products/sls/documents/log-collection-supports-nanosecond-precision-timestamps.md)。
...
其他配置保持默认，单击创建。如需了解其他参数，请参见[管理](products/sls/documents/manage-a-project.md)[Project](products/sls/documents/manage-a-project.md)。
...
其他配置保持默认，单击确定。如需了解其他参数，请参考[管理](products/sls/documents/manage-a-logstore.md)[LogStore](products/sls/documents/manage-a-logstore.md)。

### 步骤二： 配置机器组（安装LoongCollector）

在成功[创建](products/sls/documents/log-collection-supports-nanosecond-precision-timestamps.md)[Project](products/sls/documents/log-collection-supports-nanosecond-precision-timestamps.md)[和](products/sls/documents/log-collection-supports-nanosecond-precision-timestamps.md)[LogStore](products/sls/documents/log-collection-supports-nanosecond-precision-timestamps.md)后，为服务器安装LoongCollector并将其加入机器组。本文以ECS实例安装LoongCollector（ECS与日志服务Project属于同一阿里云账号和地域）为例，若ECS实例与Project不属于同一账号或地域，或为自建服务器请参考[安装采集器](products/sls/documents/loongcollector-installation-linux.md)手动安装。
...
完成[LoongCollector](products/sls/documents/log-collection-supports-nanosecond-precision-timestamps.md)[安装和机器组配置](products/sls/documents/log-collection-supports-nanosecond-precision-timestamps.md)后，进入Logtail配置页面，开始定义日志采集和处理规则。
...
单击添加处理插件，选择原生处理插件>[时间解析](products/sls/documents/time-parsing.md)，进行如下配置：
...
时间格式：根据日志的时间字段内容设置对应的[时间格式](products/sls/documents/log-collection-supports-nanosecond-precision-timestamps.md)，本示例为%Y-%m-%d %H:%M:%S,%f。其中%f为秒的小数部分，精度最高支持为纳秒。
...
单击添加处理插件，选择拓展处理插件>[提取日志时间（strptime](products/sls/documents/extract-log-time.md)[时间格式）](products/sls/documents/extract-log-time.md)，进行如下配置：
...
原始时间格式：根据日志的时间字段内容设置对应的[时间格式](products/sls/documents/log-collection-supports-nanosecond-precision-timestamps.md)，本示例为%Y-%m-%d %H:%M:%S,%f。其中%f为秒的小数部分，精度最高支持为纳秒。

时间格式字符串必须与原始日志中的时间格式（包括秒和纳秒之间的分隔符，如,或.）完全一致，否则无法正确解析。

## 提取日志时间（Go语言时间格式）

单击添加处理插件，选择拓展处理插件>[提取日志时间（Go](products/sls/documents/extract-log-time.md)[语言时间格式）](products/sls/documents/extract-log-time.md)，进行如下配置：
...
原始时间格式：根据原始日志的时间字段设置对应的时间格式，需要按照[Golang](products/sls/documents/log-collection-supports-nanosecond-precision-timestamps.md)[的时间格式规范](products/sls/documents/log-collection-supports-nanosecond-precision-timestamps.md)来编写。格式化时间模板为Go语言的诞生时间2006-01-02 15:04:05 -0700 MST。本示例对应的时间格式为2006-01-02 15:04:05,999999999。
...
结果时间格式：解析日志后的时间格式，按照[Golang](products/sls/documents/log-collection-supports-nanosecond-precision-timestamps.md)[的时间格式规范](products/sls/documents/log-collection-supports-nanosecond-precision-timestamps.md)编写。本示例为2006-01-02 15:04:05,999999999Z07:00。
...
[查询与分析概述](products/sls/documents/log-analysis-overview.md)

- 

[SDK](products/sls/documents/high-precision-timestamp-and-global-sorting.md)[操作高精度时间戳](products/sls/documents/high-precision-timestamp-and-global-sorting.md)
...
[上一篇：如何采集多种格式的日志](products/sls/documents/how-do-i-collect-different-types-of-logs-in-full-regex-mode.md)[下一篇：如何配置日志服务的时间转换格式](products/sls/documents/how-to-configure-the-time-conversion-format-of-log-service.md)
