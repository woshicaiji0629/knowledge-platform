# 如何创建并使用K8s事件中心-日志服务(SLS)-阿里云帮助中心

Source: https://help.aliyun.com/zh/sls/create-and-use-an-event-center
...
调整存储时间为105天，且K8s集群每天产生1000条事件，则超过90天后，K8s事件中心会产生LogStore存储费用（计费项为存储空间-日志存储），每天费用约0.1元。关于存储空间-日志存储计费项的更多信息，请参见[按使用功能计费模式计费项](products/sls/documents/billable-items.md)。

## 产品试用

SLSPlayground中的K8s事件中心Demo，内置了演示数据、可视化图表等资源，提供了完整的演示环境，便于您快速了解及体验功能。

您可以单击[K8s](https://sls.aliyun.com/doc/playground/demo.html?dest=/lognext/app/k8s-event/setting)[事件中心](https://sls.aliyun.com/doc/playground/demo.html?dest=/lognext/app/k8s-event/setting)，进行试用。
...
如果是ACK集群，则对应阿里云Kubernetes组件中的ack-node-problem-detector组件已集成eventer和node-problem-detector功能，您只需要部署该组件。更多信息，请参见[事件监控](products/ack/documents/ack-managed-and-ack-dedicated/user-guide/event-monitoring.md)。如果是ACK Serverless集群，您需要部署kube-eventer组件。
...
如果在创建集群时，已选中安装 node-problem-detector 并创建事件中心，可直接按照[步骤二](products/ack/documents/ack-managed-and-ack-dedicated/user-guide/event-monitoring.md)查看Kubernetes事件中心。关于如何通过创建集群时安装NPD组件，请参见[创建](products/ack/documents/ack-managed-and-ack-dedicated/user-guide/create-an-ack-managed-cluster-2.md)[ACK](products/ack/documents/ack-managed-and-ack-dedicated/user-guide/create-an-ack-managed-cluster-2.md)[托管集群](products/ack/documents/ack-managed-and-ack-dedicated/user-guide/create-an-ack-managed-cluster-2.md)。

- 

若创建集群时未选中安装 node-problem-detector 并创建事件中心，则需手动安装，具体的操作步骤如下。
...
安装kubectl工具。具体操作，请参见[获取集群](products/ack/documents/serverless-kubernetes/user-guide/connect-to-an-ack-cluster-by-using-kubectl.md)[KubeConfig](products/ack/documents/serverless-kubernetes/user-guide/connect-to-an-ack-cluster-by-using-kubectl.md)[并通过](products/ack/documents/serverless-kubernetes/user-guide/connect-to-an-ack-cluster-by-using-kubectl.md)[kubectl](products/ack/documents/serverless-kubernetes/user-guide/connect-to-an-ack-cluster-by-using-kubectl.md)[工具连接集群](products/ack/documents/serverless-kubernetes/user-guide/connect-to-an-ack-cluster-by-using-kubectl.md)。
...
| 配置项 | 类型 | 是否必选 | 说明 |
| --- | --- | --- | --- |
| endpoint | string | 必选 | 日志服务的 Endpoint。更多信息，请参见 [服务入口](products/sls/documents/developer-reference/service-entrance.md) 。 |
...
| regionId | string | 自建 Kubernetes：必选。 | 日志服务所在地域 ID。更多信息，请参见 [服务入口](products/sls/documents/developer-reference/service-entrance.md) 。 |
| accessKeyId | string | 自建 Kubernetes：必选。 | AccessKey ID，建议使用 RAM 用户的 AccessKey 信息。更多信息，请参见 [访问密钥](products/sls/documents/developer-reference/access-key.md) 。 |
| accessKeySecret | string | 自建 Kubernetes：必选。 | AccessKey Secret，建议使用 RAM 用户的 AccessKey 信息。更多信息，请参见 [访问密钥](products/sls/documents/developer-reference/access-key.md) 。 |
...
| 设置告警 | 在 告警配置 页面，您可以为 K8s 事件中心设置告警。具体操作，请参见 [设置告警](products/sls/documents/configure-alerts-2-k8s.md) 。 |
| 自定义查询 | 在 自定义查询 页面，您可以自定义查询和分析语句。 K8s 事件中心的所有事件都保存在 LogStore 中，您可以使用 LogStore 中的所有功能，例如自定义查询、消费事件、创建自定义报表、创建自定义告警等。更多信息，请参见 [查询与分析快速指引](products/sls/documents/quick-guide-to-query-and-analysis.md) 。 如果您要访问 K8s 事件中心所在的 Project，可通过以下两种方式获取 Project 名称。 通过 自定义查询 页面的 URL 定位到 Project。URL 规则为 https://sls.console.aliyun.com/lognext/app/k8s-event/project/k8s-log-xxxx/logsearch/k8s-event ，Project 字段的后一个字段即为日志服务 Project 名称，例如 k8s-log-xxxx。 在 事件中心管理 页签的 K8s 事件中心列表中，查看目标 K8s 事件中心实例对应的 Project 名称。 |
| 更新版本 | 在 版本更新 页面，您可以升级 K8s 事件中心的版本。 |


## 删除K8s事件中心实例

在K8S事件中心>事件中心管理页面，单击实例名称后操作列的图标。
...
检查并修改参数配置。更多信息，请参见[步骤一：部署](products/sls/documents/create-and-use-an-event-center.md)[eventer](products/sls/documents/create-and-use-an-event-center.md)[和](products/sls/documents/create-and-use-an-event-center.md)[node-problem-detector](products/sls/documents/create-and-use-an-event-center.md)。

- 

如果您使用的是自建Kubernetes集群，参数配置请参见[采集](products/sls/documents/collect-kubernetes-events.md)[Kubernetes](products/sls/documents/collect-kubernetes-events.md)[事件](products/sls/documents/collect-kubernetes-events.md)。
...
[上一篇：K8s事件中心](products/sls/documents/k8s-event-center.md)[下一篇：设置告警](products/sls/documents/configure-alerts-2-k8s.md)
