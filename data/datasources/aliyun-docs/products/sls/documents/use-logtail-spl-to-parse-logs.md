...
已开通日志服务。具体操作，请参见[存储资源层级关系说明](products/sls/documents/resource-management-overview.md)。
...
SPL 使用示例请参见[使用](products/sls/documents/use-spl-to-collect-text-logs.md)[SPL](products/sls/documents/use-spl-to-collect-text-logs.md)[采集文本日志](products/sls/documents/use-spl-to-collect-text-logs.md)。

SPL 使用场景请参见[数据处理最佳实践](products/sls/documents/spl-data-processing-best-practices.md)。
...
| 日志主题类型 | 选择日志主题（Topic）的生成方式。更多信息，请参见 [日志主题](products/sls/documents/log-topics.md) 。 机器组 Topic ：设置为机器组的 Topic 属性，用于明确区分不同机器组产生的日志。 文件路径提取 ：设置为文件路径正则，则需要设置 自定义正则 ，用正则表达式从路径里提取一部分内容作为 Topic。用于区分不同源产生的日志。 自定义： 自定义日志主题。 |
...
| 处理模式 | 处理模式 选择 SPL 。 |
| SPL 语句 | SPL 语句具体请参考 [SPL](products/sls/documents/user-guide/spl-syntax.md) [语法](products/sls/documents/user-guide/spl-syntax.md) 。解析日志前，日志默认存在 content 字段中。 |
...
[上一篇：时间处理类插件](products/sls/documents/time-processing-class-plug-in.md)[下一篇：数据写入时处理（写入处理器）](products/sls/documents/sls-write-processor.md)
