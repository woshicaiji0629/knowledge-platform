...
添加标注：配置告警的标题和描述等标注信息，您可以在标注信息中引用字段变量（例如${host}）。更多信息，请参见[添加标签和标注](products/sls/documents/labels-and-annotations.md)。
...
添加标注：配置告警的标题和描述等标注信息，您可以在标注信息中引用字段变量（例如${pv}）。更多信息，请参见[添加标签和标注](products/sls/documents/labels-and-annotations.md)。

其中，title设置为${bucket} Bucket发生500错误，desc设置为错误次数为${pv}。

[上一篇：设置查询统计语句](products/sls/documents/set-query-statistics-statement.md)[下一篇：设置触发条件](products/sls/documents/set-trigger-conditions.md)
