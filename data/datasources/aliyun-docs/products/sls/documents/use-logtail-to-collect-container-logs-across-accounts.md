...
其中用户自定义标识需设置为您在[步骤一：设置阿里云账号为用户标识](products/sls/documents/use-logtail-to-collect-container-logs-across-accounts.md)中获取的机器组标识（例如k8s-group-cc47****54428）。其他参数说明，请参见[创建用户自定义标识机器组](products/sls/documents/create-a-user-defined-identity-machine-group.md)。名称设置为k8s-group，机器组标识选择用户自定义标识，在用户自定义标识输入框中填写自定义标识值（如k8s-group-cc{实例标识}）。
...
心跳为OK表示容器节点与日志服务的连接正常。如果显示FAIL请参见[Logtail](products/sls/documents/troubleshoot-the-errors-related-to-logtail-machine-groups.md)[机器组无心跳](products/sls/documents/troubleshoot-the-errors-related-to-logtail-machine-groups.md)。确认所有节点的心跳状态均显示为OK，表示机器组创建成功。
...
选中您在[步骤二：创建机器组](products/sls/documents/use-logtail-to-collect-container-logs-across-accounts.md)中所创建的机器组，将该机器组从源机器组移动到应用机器组，单击下一步。

- 

设置Logtail采集配置，单击下一步。

具体参数说明，请参见[通过](products/sls/documents/use-the-log-service-console-to-collect-container-text-logs-in-daemonset-mode.md)[DaemonSet-控制台方式采集容器文本日志](products/sls/documents/use-the-log-service-console-to-collect-container-text-logs-in-daemonset-mode.md)。
...
[上一篇：将其他阿里云账号ECS上的日志集中采集到日志服务](products/sls/documents/use-logtail-to-collect-logs-across-accounts.md)[下一篇：通过Logtail跨地域采集日志](products/sls/documents/cross-region-log-collection-through-logtail-and-alibaba-cloud-account.md)
