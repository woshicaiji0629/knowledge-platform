...
下面我们以[CLB](products/slb/documents/classic-load-balancer/user-guide/clb-access-logs.md)[访问日志](products/slb/documents/classic-load-balancer/user-guide/clb-access-logs.md)为例创建一个仪表盘，用于监控不同请求方法的PV趋势，并使用过滤器查看指定方法的PV趋势。日志样例如下：
...
本文介绍了单个统计图表的创建步骤，您可以将相关的多个统计图表配置在同一个仪表盘中方便查看，配置仪表盘的步骤和更多的统计图表类型，请参见[使用仪表盘](products/sls/documents/dashboard-real-time-monitoring-and-visualization.md)。

- 

创建仪表盘后，可以分享到钉钉账号、企业微信账号或阿里云账号，或者通过免密链接在浏览器中直接访问，具体操作步骤请参见[免密分享与集成仪表盘](products/sls/documents/secret-free-sharing-and-integrated-dashboard.md)。

- 

在仪表盘发现异常时，当您需要快速进一步分析，例如在相应LogStore中进行查询分析、打开Trace分析页面或其他仪表盘页面，可以使用交互事件功能，配置步骤请参见[使用仪表盘下钻分析定位异常根因](products/sls/documents/user-guide/use-instrument-cluster-drill-in-analysis-to-locate-the-root-cause.md)。

- 

在日常仪表盘使用中，我们往往需要及时发现统计图表上指标异常变化。为仪表盘配置图表阈值高亮和告警，操作步骤请参见[使用仪表盘快速发现异常指标](products/sls/documents/use-dashboards-to-quickly-discover-abnormal-metrics.md)。

[上一篇：仪表盘](products/sls/documents/dashboard.md)[下一篇：使用仪表盘](products/sls/documents/dashboard-real-time-monitoring-and-visualization.md)
