...
扩容操作本身不收取费用，但扩容后的资源将按照[标准价格](products/sls/documents/billing-overview.md)进行计费。

- 

扩容操作请参见[管理资源配额](products/sls/documents/manage-a-project.md)。
...
| 类别 | 限制项 | 说明 | 备注 |
| --- | --- | --- | --- |
| [Project](products/sls/documents/manage-a-project.md) | 写入流量 | 原始数据写入流量最大为 100GB/min。 | 超过限制时，返回状态码 403，提示 Inflow Quota Exceed。 |
...
| [Shard](products/sls/documents/manage-shards.md) | 写入流量 | 每个 Shard 支持 5MB/s 的数据写入。 | 非硬性限制。超过限制时，系统会尽可能提供服务，但不保证服务质量。 |
...
[上一篇：资源管理](products/sls/documents/resource-management.md)[下一篇：管理Project](products/sls/documents/manage-a-project.md)
