...
该Logstore默认开启索引，并配置部分字段的索引。您可以增加索引字段，修改索引后只对新数据生效。您还可以对历史数据重建索引。具体操作，请参见[重建索引](products/sls/documents/reindex-logs-for-a-logstore.md)。

- 

您可以自定义修改日志存储时间。具体操作，请参见[修改](products/sls/documents/manage-a-logstore.md)[Logstore](products/sls/documents/manage-a-logstore.md)[配置](products/sls/documents/manage-a-logstore.md)。
...
日志服务根据存储空间、读取流量、请求数量、数据加工、数据投递等进行收费。更多信息，请参见[按使用功能计费模式计费项](products/sls/documents/billable-items.md)。
...
[上一篇：设置告警](products/sls/documents/configure-alerts-2-k8s.md)[下一篇：配置Ingress日志中心](products/sls/documents/configure-ingress-log-center.md)
