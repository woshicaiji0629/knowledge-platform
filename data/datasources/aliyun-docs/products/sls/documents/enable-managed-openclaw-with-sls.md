...
已[创建](products/sls/documents/manage-a-project.md)[Project](products/sls/documents/manage-a-project.md)（如openclaw-observability）且[已创建](products/sls/documents/manage-a-logstore.md)[LogStore](products/sls/documents/manage-a-logstore.md)。

- 

确保 OpenClaw 所在 ECS / 自建服务器已[安装](products/sls/documents/loongcollector-installation-linux.md)[LoongCollector](products/sls/documents/loongcollector-installation-linux.md)。
...
若机器组心跳异常请参考[心跳异常问题汇总排查](products/sls/documents/troubleshooting-of-abnormal-heartbeat-problems.md)。
...
以千问3.5-Plus模型为例，百炼API调用的费用见[模型列表](products/model-studio/documents/models.md)。
...
Session 日志— 记录 Agent 的完整业务行为，是安全审计与成本分析的主要依据。即在[步骤一：日志接入（以 Session 日志为例）](products/sls/documents/enable-managed-openclaw-with-sls.md)中接入的日志。
...
选择OpenClaw-运行时日志卡片并参考[步骤一：日志接入（以 Session 日志为例）](products/sls/documents/enable-managed-openclaw-with-sls.md)进行接入。
...
[上一篇：AI Agent](products/sls/documents/ai-agent.md)[下一篇：数据格式定制化处理](products/sls/documents/data-format-customization.md)
