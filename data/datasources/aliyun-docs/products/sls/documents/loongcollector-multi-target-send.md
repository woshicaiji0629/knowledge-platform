...
选择机器组：选择目标主机所在的机器组并添加到应用机器组。如目标主机尚未安装LoongCollector，请参考[配置机器组（安装](products/sls/documents/host-text-log-collection-auto-install.md)[LoongCollector）](products/sls/documents/host-text-log-collection-auto-install.md)完成机器组配置。
...
如需按字段进行精确查询，请在页面加载出预览数据后，单击自动生成索引，日志服务将根据预览数据中的第一条内容生成[创建索引](products/sls/documents/create-indexes.md)。
...
[上一篇：通过Logtail跨地域采集日志](products/sls/documents/cross-region-log-collection-through-logtail-and-alibaba-cloud-account.md)[下一篇：Logtail最佳实践](products/sls/documents/best-practices-for-logtail.md)
