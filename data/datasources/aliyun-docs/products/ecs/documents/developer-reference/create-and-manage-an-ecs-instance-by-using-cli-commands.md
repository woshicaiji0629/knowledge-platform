...
调用[CreateVpc](products/vpc/documents/developer-reference/api-vpc-2016-04-28-createvpc.md)创建VPC。
...
调用[CreateVSwitch](products/vpc/documents/developer-reference/api-vpc-2016-04-28-createvswitch.md)在VPC中创建交换机。
...
调用[AuthorizeSecurityGroup](products/ecs/documents/developer-reference/api-ecs-2014-05-26-authorizesecuritygroup.md)添加安全组规则。
...
调用[RunInstances](products/ecs/documents/developer-reference/api-ecs-2014-05-26-runinstances.md)创建一个包年包月的ECS实例。
...
| SecurityGroupId | 安全组 ID：根据 [CreateSecurityGroup](products/ecs/documents/api-createsecuritygroup.md) 返回结果。 示例： sg-bp18z2q1jg4gq95t**** |
| VSwitchId | 交换机 ID：根据 [CreateVSwitch](products/vpc/documents/api-createvswitch.md) 返回结果。 示例： vsw-bp11hf5r945gewysp**** |
...
调用[DescribeInstances](products/ecs/documents/developer-reference/api-ecs-2014-05-26-describeinstances.md)，通过实例ID（i-bp1ducce5hs1jm98****）获取实例的公网IP信息。
...
调用[StartInstance](products/ecs/documents/api-startinstance.md)接口启动一台ECS实例。
...
您可以调用[DescribeInstances](products/ecs/documents/api-describeinstances.md)接口查询一台或多台ECS实例的详细信息。
...
调用[DescribeInstances](products/ecs/documents/api-describeinstances.md)分页查询杭州地域的ECS实例，每页展示5条信息。
...
调用[CreateSnapshot](products/ecs/documents/api-createsnapshot.md)接口创建快照。
...
调用[CreateImage](products/ecs/documents/api-createimage.md)接口，基于ECS实例创建一个自定义镜像。
...
调用[StopInstance](products/ecs/documents/api-stopinstance.md)停止一台运行中（Running）的ECS实例，正常关机（ForceStop为 false）且停机模式为普通停机模式（StoppedMode为KeepCharging，即停止后仍旧保留实例并继续收费），预检查后正常停止ECS实例。
...
本示例仅使用部分API，更多ECS API信息，请参见[API](products/ecs/documents/developer-reference/api-ecs-2014-05-26-overview.md)[概览](products/ecs/documents/developer-reference/api-ecs-2014-05-26-overview.md)。

在阿里云CLI中，您可根据需要使用命令行选项，用来修改命令的默认行为或为命令提供额外功能，请参见[命令行选项](https://help.aliyun.com/zh/cli/command-line-options)。

[上一篇：PHP SDK调用示例](products/ecs/documents/developer-reference/example-on-how-to-use-ecs-sdk-for-php.md)[下一篇：通过CLI创建并使用ECS实例](products/ecs/documents/developer-reference/cli-reference.md)
