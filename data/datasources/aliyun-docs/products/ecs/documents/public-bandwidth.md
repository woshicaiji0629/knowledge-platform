...
本文仅介绍固定公网IPv4的两种计费模式。如果实例通过其他方式（如IPv6，EIP，NAT等）访问公网，请查看[对应产品的计费规则](products/ecs/documents/public-bandwidth.md)。
...
| 实例计费方式 | 配置参数及单价 | 费用计算 |
| --- | --- | --- |
| 包年包月 | 带宽值：10Mbps 使用时长：1 个月 单位时长带宽值价格： 525 元/月 参考 [如何计算单位时长不同带宽值的单价](products/ecs/documents/public-bandwidth.md) | 公网带宽费用为 525 * 1 = 525 元 |
| 按量付费、抢占式实例 | 带宽值：10Mbps 使用时长：1 个月（720 小时） 单位时长带宽值价格： 1.5625 元/小时 参考 [如何计算单位时长不同带宽值的单价](products/ecs/documents/public-bandwidth.md) | 公网带宽费用为 1.5625 * 720 = 1125 元 |
...
| 公式参数 | 参数说明 |
| --- | --- |
| 出网流量 | ECS 使用时实际的出网流量，可按照计费周期维度 [查看云服务器](products/ecs/documents/billing-faq.md) [ECS](products/ecs/documents/billing-faq.md) [公网流量的使用量](products/ecs/documents/billing-faq.md) ，精确到 Byte，转换为 GB 单位计算时会取整，请以账单最终显示为准。 |
...
按固定带宽计费时，定期[监控网络带宽](products/ecs/documents/user-guide/network-bandwidth.md)，根据业务情况评估并灵活调整带宽值是节省成本的关键方法。
...
| 按固定带宽计费 | 闲置带宽较多时 [降低公网带宽配置](products/ecs/documents/user-guide/modify-the-bandwidth-configurations.md) | 业务的实际带宽峰值长期远低于您购买的带宽值，降低带宽规格可以直接减少固定的带宽费用，避免为大量闲置的带宽资源付费。 |
| 带宽升配需求时采用临时升配替代长期升配 该方案当前仅适用于包年包月实例。 | 在带宽升配需求可预期时间时，可以通过 [临时升级固定公网带宽](products/ecs/documents/user-guide/temporary-bandwidth-upgrade.md) ，及时帮临时升配后回落至常态的带宽值，避免为大量闲置的带宽资源付费。 如果需要周期性重复临时升级操作，可以通过 [自动化运维](products/ecs/documents/user-guide/temporary-upgrade-bandwidth-on-a-daily-basis.md) [OOS](products/ecs/documents/user-guide/temporary-upgrade-bandwidth-on-a-daily-basis.md) [便捷实现](products/ecs/documents/user-guide/temporary-upgrade-bandwidth-on-a-daily-basis.md) 。 |  |


### 转换计费模式

建议在业务变化后，定期[监控网络带宽](products/ecs/documents/user-guide/network-bandwidth.md)的情况。您可以结合[推荐使用场景](products/ecs/documents/public-bandwidth.md)，重新评估当前的计费模式是否仍然适用于您的业务场景。若评估后判断需要更换计费方式，可以参考[转换固定公网](products/ecs/documents/change-the-billing-method-for-network-usage-1.md)[IP](products/ecs/documents/change-the-billing-method-for-network-usage-1.md)[的带宽计费方式](products/ecs/documents/change-the-billing-method-for-network-usage-1.md)操作。
...
不需要使用固定公网带宽时，可以通过将固定公网带宽值调整为0 Mbit/s的方式，实现固定公网IP的释放，进而实现实例公网带宽停止计费，操作步骤请参见[修改固定公网带宽](products/ecs/documents/user-guide/modify-the-bandwidth-configurations.md)。
...
绑定了弹性公网IP（EIP）：请参见[EIP](products/eip/documents/billing-overview.md)[计费概述](products/eip/documents/billing-overview.md)。

- 

使用NAT网关：请参见[NAT](products/nat-gateway/documents/nat-gateway-billing.md)[网关计费说明](products/nat-gateway/documents/nat-gateway-billing.md)。
...
[上一篇：块存储计费](products/ecs/documents/block-storage-devices.md)[下一篇：快照计费](products/ecs/documents/snapshots-1.md)
