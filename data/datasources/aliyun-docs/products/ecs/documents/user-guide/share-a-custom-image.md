...
（条件必选）共享加密镜像授权准备：需参见[跨账号共享加密资源](products/ecs/documents/user-guide/encryption-related-permissions.md)创建AliyunECSShareEncryptImageDefaultRole角色并授权。
...
可以调用API接口[管理镜像共享权限](products/ecs/documents/developer-reference/api-ecs-2014-05-26-modifyimagesharepermission.md)将当前阿里云账号的自定义镜像共享给其他阿里云账号。
...
可共享的账号数量受配额限制。共享行为不占用接收方的自定义镜像额度。具体限额可通过[配额中心](https://quotas.console.aliyun.com/products/ecs/quotas?spm=a2c4g.11186623.0.0.376656addmG73f)查询每个自定义镜像能够共享用户数限额项查看和[调整配额](products/ecs/documents/user-guide/quota-management.md)。
...
可以调用API接口[DescribeImageSharePermission](products/ecs/documents/api-describeimagesharepermission.md)查询自定义镜像已经共享的所有用户。

如何跨地域共享镜像？

需参考[复制自定义镜像](products/ecs/documents/user-guide/copy-an-image.md)，将待共享镜像复制到目标地域后再共享。

如何删除共享中的镜像？

共享中的镜像不支持直接删除，需要先[取消共享镜像](products/ecs/documents/user-guide/unshare-custom-images.md)，才能删除。

如何查看阿里云账号主账号的ID？

将鼠标移至控制台右上角的用户头像，在弹出的用户信息框中，如果标识的账号为主账号，则显示的账号ID即为阿里云账号ID。

[上一篇：导出自定义镜像](products/ecs/documents/user-guide/export-a-custom-image.md)[下一篇：使用共享镜像](products/ecs/documents/user-guide/use-shared-images.md)
