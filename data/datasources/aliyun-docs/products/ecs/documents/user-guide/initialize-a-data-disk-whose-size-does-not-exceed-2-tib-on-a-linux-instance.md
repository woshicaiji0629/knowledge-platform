...
若无云助手检测功能，请[通过命令行初始化](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)云盘。
...
未初始化当有扩容至64 TiB需求或页面提示云助手查询或执行失败、未安装云助手，请[通过命令行初始化](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)云盘。
...
当前为临时挂载，重启后失效。为使重启后数据仍可访问，建议登录实例[配置开机自动挂载分区](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)。

已初始化当页面提示云助手查询或执行失败、未安装云助手，请[通过命令行初始化](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)云盘。
...
创建分区有助于逻辑隔离，可将不同用途的数据分隔存放，防止相互干扰或影响。如果无需创建分区可直接[创建文件系统](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)。
...
如果目标云盘存在文件系统，表示已经初始化，仅需[挂载文件系统](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)。示例中数据盘vdb的分区vdb1，存在ext4文件系统，无需创建分区及文件系统。
...
以创建GPT分区为例，MBR分区请参看[如何创建](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)[MBR](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)[分区](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)。

请参阅[如何选择分区类型和文件系统类型](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)，了解不同分区的区别。
...
<设备名称>调整为[步骤](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)[2.a](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)中获取到的设备名称。
...
<设备名称>调整为[步骤](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)[2.a](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)中获取到的设备名称，<分区编号>为上一步获取的分区Number。

sudo parted /dev/<设备名称> align-check optimal <分区编号>

返回aligned表示对齐。[创建](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)[GPT](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)[分区时，显示](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)[not aligned](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)[分区未对齐如何解决？](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)
...
执行sudo lsblk查看新分区信息。若待初始化云盘存在正确分区信息，表明分区创建完成。接下来需要[创建文件系统](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)，才能使数据盘可用。
...
创建文件系统会删除数据盘中数据，请确保云盘为空或已[手动创建单个快照](products/ecs/documents/user-guide/create-a-snapshot.md)备份数据。
...
将命令中的变量<目标设备名称>替换为[上一步](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)获取的目标设备名称后执行。
...
运行sudo lsblk -f，若回执中目标设备的FSTYPE为所创建的目标文件系统类型，表示配置成功。接下来需要[挂载文件系统](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)，才能正常存储数据。
...
| 参数 | 说明 |
| --- | --- |
| <目标设备名称> | 替换为创建文件系统时获取的 [目标设备名称](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md) 。 |
...
当前为临时挂载，重启后失效。为使重启后数据仍可访问，建议[配置开机自动挂载分区](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)。
...
将命令中的变量<目标设备名称>、<挂载目录>和<文件系统类型>，替换为从[上一步](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)中获取到的信息后执行。
...
<目标设备名称>需替换为[步骤](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)[3.a](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)中获取的目标设备名称。
...
若需扩容已有云盘，请参考[扩容云盘（Linux）](products/ecs/documents/user-guide/resize-linux-cloud-disks.md)。

- 

若想为Windows实例初始化数据盘，请参考[初始化数据盘（Windows）](products/ecs/documents/user-guide/initialize-a-data-disk-up-to-2-tib-in-size-on-a-windows-instance.md)。
...
创建分区有助于数据管理和逻辑隔离，如果无需创建分区可直接[选择需要创建的文件系统类型](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)。
...
执行sudo lsblk查看新分区信息。若待初始化云盘存在正确分区信息，表明分区创建完成。示例中设备vdb，存在1个分区vdb1。接下来需要[创建文件系统](products/ecs/documents/user-guide/initialize-a-data-disk-whose-size-does-not-exceed-2-tib-on-a-linux-instance.md)，才能使数据盘可用。
...
可依照[Linux](products/ecs/documents/support/the-etc-fstab-file-of-the-linux-instance-is-incorrectly-configured-causing.md)[实例的/etc/fstab](products/ecs/documents/support/the-etc-fstab-file-of-the-linux-instance-is-incorrectly-configured-causing.md)[文件配置错误导致系统启动异常问题处理](products/ecs/documents/support/the-etc-fstab-file-of-the-linux-instance-is-incorrectly-configured-causing.md)，使用VNC进行远程连接实例，在紧急模式下，手动修改错误的挂载信息。
...
调用[RunCommand](products/ecs/documents/developer-reference/api-ecs-2014-05-26-runcommand.md)接口向目标实例发送初始化指令，搭配调用[DescribeInvocations](products/ecs/documents/developer-reference/api-ecs-2014-05-26-describeinvocations.md)接口查询命令回执实现初始化并挂载文件系统操作。

- 安装初始化工具时，提示“404 Not Found”怎么解决？

CentOS 6、Debian 9/10/11操作系统已结束生命周期，需要先[切换](products/ecs/documents/user-guide/options-for-dealing-with-centos-linux-end-of-life.md)[Centos](products/ecs/documents/user-guide/options-for-dealing-with-centos-linux-end-of-life.md)[源地址](products/ecs/documents/user-guide/options-for-dealing-with-centos-linux-end-of-life.md)或[Debian 9/10/11](products/ecs/documents/user-guide/other-operating-systems.md)[源地址](products/ecs/documents/user-guide/other-operating-systems.md)后再进行工具安装

[上一篇：初始化数据盘概述](products/ecs/documents/user-guide/overview-27.md)[下一篇：初始化数据盘（Windows）](products/ecs/documents/user-guide/initialize-a-data-disk-up-to-2-tib-in-size-on-a-windows-instance.md)
