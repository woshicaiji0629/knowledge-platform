...
购买突发性能实例时支持按量付费、包年包月、抢占式实例等计费方式。不同计费方式的区别，请参见[计费方式概述](products/ecs/documents/overview-of-billing-methods.md)。详细的实例规格定价，请参见[详细价格总览](https://www.aliyun.com/price/product#/ecs/detail)。

如果实例计费方式为按量付费，您可以购买预留实例券抵扣实例的按量账单，更多信息，请参见[预留实例券概述](products/ecs/documents/user-guide/overview-6.md)。但是，如果预留实例券的实例规格为t5，存在以下限制：
...
说明购买或使用突发性能实例时，您可以选择是否打开无性能约束模式。关于无性能约束模式下CPU积分的消耗规则，请参见[无性能约束模式](products/ecs/documents/user-guide/burst-performance-instance-overview.md)。
...
[上一篇：CPU积分变化示例](products/ecs/documents/user-guide/cpu-credit-change-examples.md)[下一篇：创建突发性能实例](products/ecs/documents/user-guide/create-a-burstable-instance.md)
