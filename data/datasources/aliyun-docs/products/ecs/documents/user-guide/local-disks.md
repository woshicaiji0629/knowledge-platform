...
具体操作，请参见[备份本地盘文件](products/ecs/documents/user-guide/backup-local-disk-data.md)。
...
购买了本地盘实例后，请先登录ECS实例初始化本地盘。具体操作可以参考云盘初始化操作：[初始化数据盘](products/ecs/documents/user-guide/initialize-a-data-disk.md)。
...
本文主要描述当前与ECS实例一起销售的本地盘的信息。有关本地SSD型实例规格和大数据型实例规格的性能详情，请参见[实例规格族](products/ecs/documents/user-guide/overview-of-instance-families.md)。
...
有关本地盘的性能说明，请参见[块存储性能](products/ecs/documents/user-guide/block-storage-performance.md)。

## 计费说明

本地盘的费用包括在本地盘挂载的实例的费用里。更多信息，请参见[包年包月](products/ecs/documents/subscription.md)和[按量付费](products/ecs/documents/pay-as-you-go-1.md)。
...
本地盘的生命周期与它所挂载的本地盘实例相同。更多信息，请参见[实例的生命周期](products/ecs/documents/user-guide/instance-lifecycle.md)。
...
| 实例自动恢复。实例更多的恢复方式请参见 [修改实例维护属性](products/ecs/documents/user-guide/modify-instance-maintenance-attributes.md) | 否 | 是 |
| [释放实例](products/ecs/documents/user-guide/release-an-instance.md) | 否 | 否 |
...
您可以根据性能测试文档测试本地盘的带宽、IOPS以及延迟等性能指标，验证阿里云提供的标准性能数据及本地盘服务质量QoS（Quality of Service）。具体操作，请参见[本地盘性能测试命令](products/ecs/documents/user-guide/test-the-performance-of-block-storage-devices.md)。

- 

如果您使用的是已经停售的上一代本地SSD盘，请参见[上一代磁盘-本地](products/ecs/documents/user-guide/previous-generation-disks-local-ssds.md)[SSD](products/ecs/documents/user-guide/previous-generation-disks-local-ssds.md)[盘](products/ecs/documents/user-guide/previous-generation-disks-local-ssds.md)。

- 

关于如何处理本地盘系统事件，请参见[本地盘实例运维场景和系统事件](products/ecs/documents/user-guide/operations-and-maintenance-scenarios-and-system-events-for-instances-equipped-with-local-disks.md)。

[上一篇：弹性临时盘](products/ecs/documents/user-guide/elastic-ephemeral-disks.md)[下一篇：ESSD 云盘数据可靠性](products/ecs/documents/user-guide/essd-cloud-disk-data-reliability.md)
