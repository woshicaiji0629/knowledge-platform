...
通过调用[DescribeDisks](products/ecs/documents/developer-reference/api-ecs-2014-05-26-describedisks.md)，查看AttachedTime字段可判断云盘是否存在挂载历史。
...
创建快照策略本身不收费，但策略成功执行后会根据任务类型收取相关费用。可通过设置自动快照随云盘释放、建议定期删除不再使用的自动快照以及归档快照的方式[降低快照存储成本](products/ecs/documents/user-guide/reduce-snapshot-fees.md)。

- 

自动创建快照任务：所有通过策略创建的快照，会按其快照容量收取[标准快照存储费](products/ecs/documents/snapshots-1.md)。

- 

跨地域复制任务：若策略启用了跨地域复制功能，除源地域的标准快照费外，还会产生[复制快照流量费](products/ecs/documents/snapshots-1.md)和目标地域的标准快照存储费。
...
每块云盘最多支持自动快照个数请参见[快照的使用限制](products/ecs/documents/user-guide/limitations.md)。当自动快照数量达到上限时，系统会删除由策略生成的时间最早的自动快照。请根据业务需求合理规划快照的保留时间，避免重要快照被意外删除。
...
手动快照和自动快照的区别请参见[手动快照和自动快照有什么区别？](products/ecs/documents/data-protection-and-recovery-faqs.md)
...
为云盘设置自动快照策略：[ApplyAutoSnapshotPolicy](products/ecs/documents/api-applyautosnapshotpolicy.md)

- 

取消云盘设置的自动快照策略：[CancelAutoSnapshotPolicy](products/ecs/documents/api-cancelautosnapshotpolicy.md)

- 

设置自动快照随云盘释放：[ModifyDiskAttribute](products/ecs/documents/api-modifydiskattribute.md)

[上一篇：创建自动快照策略](products/ecs/documents/user-guide/create-an-automatic-snapshot-policy-1.md)[下一篇：修改自动快照策略](products/ecs/documents/user-guide/modify-an-automatic-snapshot-policy.md)
