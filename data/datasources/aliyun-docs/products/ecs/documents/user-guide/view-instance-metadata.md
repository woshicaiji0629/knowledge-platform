...
运行于ECS实例的应用，可通过元数据服务动态查询实例ID、IP等实例元数据信息，避免硬编码。为防范[SSRF](products/ecs/documents/user-guide/view-instance-metadata.md)[攻击](products/ecs/documents/user-guide/view-instance-metadata.md)导致元数据泄露，建议通过加固模式访问元数据（需先获取访问令牌），并配置实例仅允许加固模式访问，以有效规避普通模式下的安全风险。
...
[登录](products/ecs/documents/user-guide/connect-to-a-linux-instance-by-using-a-password-or-key.md)实例。
...
curl -H "X-aliyun-ecs-metadata-token: $TOKEN" http://100.100.100.200/latest/meta-data/instance-id命令末尾的instance-id代表获取实例ID，可替换为其他需要获取的[元数据项](products/ecs/documents/user-guide/view-instance-metadata.md)。例如mac（获取MAC地址）或hostname（获取主机名）。
...
[登录](products/ecs/documents/user-guide/connect-to-a-windows-instance-through-workbench.md)实例。
...
Invoke-RestMethod -Headers @{"X-aliyun-ecs-metadata-token" = $token} -Method GET -Uri http://100.100.100.200/latest/meta-data/instance-id命令末尾的instance-id代表获取实例ID，可替换为其他需要获取的[元数据项](products/ecs/documents/user-guide/view-instance-metadata.md)。例如mac（获取MAC地址）或hostname（获取主机名）。
...
在[自定义购买实例](products/ecs/documents/user-guide/create-an-instance-by-using-the-wizard.md)时，将高级选项>元数据访问模式调整为仅加固模式。

使用自定义镜像创建实例时，若无仅加固模式选项，需对镜像进行[升级](products/ecs/documents/user-guide/view-instance-metadata.md)。

## CLI

在通过[RunInstances](products/ecs/documents/developer-reference/api-ecs-2014-05-26-runinstances.md)或[CreateInstance](products/ecs/documents/developer-reference/api-ecs-2014-05-26-createinstance.md)创建实例时，可通过配置HttpEndpoint=enabled和HttpTokens=required设置实例的元数据访问模式为仅加固模式。命令示例如下：
...
在通过[RunInstances](products/ecs/documents/developer-reference/api-ecs-2014-05-26-runinstances.md)或[CreateInstance](products/ecs/documents/developer-reference/api-ecs-2014-05-26-createinstance.md)创建实例时，通过配置HttpEndpoint=enabled和HttpTokens=required设置实例的元数据访问策略为仅加固模式。
...
确保Cloud-init版本不低于23.2.2：可登录实例并执行cloud-init --version命令查看当前版本。若版本过低，切换仅加固模式后将导致实例启动异常，请先[升级](products/ecs/documents/user-guide/install-cloud-init.md)Cloud-init版本到23.2.2或更高版本。

- 

所有应用代码/脚本已通过[方式一：加固模式](products/ecs/documents/user-guide/view-instance-metadata.md)访问实例元数据。

重要

若应用代码依赖Credentials库获取STS Token配置SDK，需将Credentials依赖升级至[支持加固模式的版本](products/ecs/documents/user-guide/view-instance-metadata.md)。

升级完成后，可[如何检测](products/ecs/documents/user-guide/view-instance-metadata.md)[ECS](products/ecs/documents/user-guide/view-instance-metadata.md)[实例是否存在普通模式的元数据访问？](products/ecs/documents/user-guide/view-instance-metadata.md)，在确定不存在普通模式访问后，开启实例的仅加固模式。
...
调用[ModifyInstanceMetadataOptions](products/ecs/documents/developer-reference/api-ecs-2014-05-26-modifyinstancemetadataoptions.md)，设置HttpEndpoint=enabled、HttpTokens=required切换实例元数据访问模式为仅加固模式。命令示例：
...
调用[ModifyInstanceMetadataOptions](products/ecs/documents/developer-reference/api-ecs-2014-05-26-modifyinstancemetadataoptions.md)，设置HttpEndpoint=enabled、HttpTokens=required切换实例元数据访问模式为仅加固模式。

切换完成后，建议持续监控实例的元数据访问情况和应用运行状态，确保业务平稳运行，避免因遗漏改造的应用而导致服务中断。若出现异常，建议先切换回普通模式和加固模式，优先恢复业务，然后重新进一步[步骤一：排查并升级代码及依赖项](products/ecs/documents/user-guide/view-instance-metadata.md)。
...
| tags/instance/[tagKey] | 用于获取实例的指定标签值，其中 [tagKey] 为要查询的标签键。 使用此功能，需先调用 [ModifyInstanceMetadataOptions](products/ecs/documents/developer-reference/api-ecs-2014-05-26-modifyinstancemetadataoptions.md) 接口，将实例的 InstanceMetadataTags 参数设置为 enabled ，以启用此功能。 | dev |  |
...
可能是实例中应用或脚本中仍在使用旧的普通模式。请按照[升级到仅加固模式](products/ecs/documents/user-guide/view-instance-metadata.md)彻底排查并升级依赖普通模式的应用。
...
改造临时实例：在第一步创建的临时实例上，参考[将已有实例升级到仅加固模式](products/ecs/documents/user-guide/view-instance-metadata.md)完成以下改造：
...
创建新镜像并修改属性：改造完成后，基于这台实例创建新版自定义镜像。然后调用[ModifyImageAttribute](products/ecs/documents/developer-reference/api-ecs-2014-05-26-modifyimageattribute.md)调整镜像属性Features.ImdsSupport=v2。
...
| Alibaba Cloud Linux 3 Alibaba Cloud Linux 3 Pro Anolis OS 8 CentOS Stream 8/9 CentOS 8 | Ubuntu: 20/24 Debian:10,11,12 Fedora 35+ AlmaLinux 8/9 Rocky Linux 8/9 | Red Hat Enterprise Linux 8/9 Red Hat 中需要自行下载 rpm 包 [安装云助手](products/ecs/documents/user-guide/install-the-cloud-assistant-agent.md) [Agent](products/ecs/documents/user-guide/install-the-cloud-assistant-agent.md) 。 SUSE 15.1/15.2/15.3/15.4/15.5/15.6 OpenSuse 15.2/15.3/15.4/15.5/15.6 |
| --- | --- | --- |
...
[登录](products/ecs/documents/user-guide/connect-to-a-linux-instance-by-using-a-password-or-key.md)ECS实例，执行以下命令安装并启动监控服务，启用后会占用部分实例性能。
...
[上一篇：开启和关闭实例释放保护](products/ecs/documents/user-guide/enable-or-disable-release-protection-for-ecs-instances.md)[下一篇：实例动态数据（实例标识）](products/ecs/documents/user-guide/use-instance-identities.md)
