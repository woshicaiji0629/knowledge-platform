...
| 密码 | 若忘记或未设置密码，需先 [重置](products/ecs/documents/user-guide/instance-logon-credential-management.md) 。 为避免每次登录需要重复输入用户名和密码，可以 [记住密码](products/ecs/documents/user-guide/connect-to-a-windows-instance-through-workbench.md) ，以便之后使用。 |
...
[上一篇：使用Workbench登录Linux实例](products/ecs/documents/user-guide/connect-to-a-linux-instance-by-using-a-password-or-key.md)[下一篇：在阿里云APP上使用Workbench登录实例](products/ecs/documents/user-guide/login-using-the-workbench-instance-on-ali-cloud-app.md)
