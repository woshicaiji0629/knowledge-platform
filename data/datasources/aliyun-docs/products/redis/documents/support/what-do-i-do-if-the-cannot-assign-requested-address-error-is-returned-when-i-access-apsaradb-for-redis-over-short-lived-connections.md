...
参数说明：[$Hostname]、[$Port]和[$Inst_Password]分别为Redis实例的连接地址、端口号和密码，如何查看请参见[查看连接地址](products/redis/documents/user-guide/view-endpoints.md)。
...
[云数据库 Tair（兼容 Redis）](products/redis/documents/product-overview/what-is-apsaradb-for-redis.md)

[上一篇：Destination Host Unreachable报错](products/redis/documents/support/destination-host-unavailable.md)[下一篇：为什么没有加入IP白名单的客户端可以访问实例？](products/redis/documents/support/why-are-clients-still-able-to-access-an-instance-even-when-an-instance-whitelist-does-not-include-the-client-ip-addresses.md)
