...
[磁盘型](products/redis/documents/product-overview/essd-based-instances-1.md)实例在小版本2.7.0之前不支持此功能，可[升级小版本](products/redis/documents/user-guide/update-the-minor-version.md)至最新版本使用此功能。
...
A：当业务中数据设置了过期时间，且实例在同一时间（时间段）过期大量Key时，可能会出现该情况。此时，除了依靠实例自动清除过期数据外，您可以通过控制台上的清除数据功能快速清除过期Key，更多信息请参见[如何清除过期](products/redis/documents/support/how-do-i-clear-expired-keys.md)[Key](products/redis/documents/support/how-do-i-clear-expired-keys.md)。

- 

Q：若使用RAM账号，操作时提示权限不足怎么办？

A：请对RAM账号进行授权并重试，更多信息请参见[常见自定义权限策略场景及示例](products/redis/documents/custom-permission-policy-reference.md)。
...
[Top Key](products/redis/documents/user-guide/use-the-real-time-key-statistics-feature.md)[统计](products/redis/documents/user-guide/use-the-real-time-key-statistics-feature.md)

[上一篇：Top Key统计](products/redis/documents/user-guide/use-the-real-time-key-statistics-feature.md)[下一篇：通过读写分离缓解热点Key问题](products/redis/documents/user-guide/use-read-write-splitting-to-mitigate-issues-caused-by-hotkeys.md)
