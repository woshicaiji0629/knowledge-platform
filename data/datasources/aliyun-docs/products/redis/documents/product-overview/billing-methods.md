...
| 按量付费+资源包 | 灵活、低价 | 计费周期、计费规则均与按量付费相同，但费用由预购的资源包自动抵扣。 按量付费的 经典 版 Tair（企业版） 实例或 经典 版 Redis 开源版 实例的实例规格费用，优先从资源包抵扣。若资源包剩余额度不足，超出部分的费用仍以按量付费的形式从账户中扣除，更多信息，请参见 [资源包](products/redis/documents/product-overview/resource-plan-overview.md) 。 |
...
[上一篇：计费项](products/redis/documents/product-overview/billable-items.md)[下一篇：资源包](products/redis/documents/product-overview/resource-plan-overview.md)
