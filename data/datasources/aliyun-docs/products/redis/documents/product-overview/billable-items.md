...
| 计费项 | 说明 |
| --- | --- |
| [实例规格费用](products/redis/documents/product-overview/billable-items.md) | [创建实例](products/redis/documents/getting-started/step-1-create-an-apsaradb-for-redis-instance.md) 时，根据实例规格和购买时长产生费用，支持包年包月、按量付费的计费方式。 按量付费实例推荐搭配资源包，降低成本。 更多关于计费方式的信息，请参见 [计费方式](products/redis/documents/product-overview/billing-methods.md) 。 说明 该项费用中包含磁盘费用，磁盘费用不可调整，磁盘用于系统运行，例如存储日志、备份临时文件、AOF 文件等。 |
| [其他费用](products/redis/documents/product-overview/billable-items.md) （非必须） | 备份费用：当备份文件用量超过免费额度后，超出部分将按量收费。 审计日志：开通后，将根据审计日志占用的存储空间和保存时间，按小时出账。 带宽升级：系统将根据升级带宽的大小和使用时间，按小时出账。 |
...
免费额度 = 实例规格默认内存大小（Tair[磁盘型](products/redis/documents/product-overview/essd-based-instances-1.md)以磁盘存储空间为准）
...
[上一篇：产品计费](products/redis/documents/product-overview/billing.md)[下一篇：计费方式](products/redis/documents/product-overview/billing-methods.md)
