...
已创建ACK托管集群Pro版，且已配置节点池的CPU Policy（cpuManagerPolicy）为none。请参见[自定义节点池](products/ack/documents/ack-managed-and-ack-dedicated/user-guide/customize-the-kubelet-configurations-of-a-node-pool.md)[kubelet](products/ack/documents/ack-managed-and-ack-dedicated/user-guide/customize-the-kubelet-configurations-of-a-node-pool.md)[配置](products/ack/documents/ack-managed-and-ack-dedicated/user-guide/customize-the-kubelet-configurations-of-a-node-pool.md)配置cpuManagerPolicy取值。

- 

已安装[ack-koordinator](products/ack/documents/product-overview/ack-koordinator-fka-ack-slo-manager.md)，且组件版本为0.2.0及以上。
...
[登录](products/ack/documents/ack-managed-and-ack-dedicated/user-guide/overview-of-node-management.md)[Pod](products/ack/documents/ack-managed-and-ack-dedicated/user-guide/overview-of-node-management.md)[所在节点](products/ack/documents/ack-managed-and-ack-dedicated/user-guide/overview-of-node-management.md)，获取Pod的UID和Container ID。
...
[普通绑核策略](products/ack/documents/ack-managed-and-ack-dedicated/user-guide/topology-aware-cpu-scheduling.md)：通用策略，遵循1:1的绑核原则，为Pod绑定resources.limits.cpu所指定的数量的核心。优先选择同一NUMA节点内的CPU核，以保证良好的内存访问性能。

- 

[自动绑核策略](products/ack/documents/ack-managed-and-ack-dedicated/user-guide/topology-aware-cpu-scheduling.md)：针对特定硬件优化的策略，优先通过绑定一个完整的物理核心簇（如AMD CPU的CCX/CCD）来最大化CPU局部性并提升并发度。推荐在特定的大规格（如32核及以上）AMD机型上使用。
...
等待Pod正常运行后，再次[登录其所在节点](products/ack/documents/ack-managed-and-ack-dedicated/user-guide/overview-of-node-management.md)，查看CPU绑核情况。
...
ack-koordinator默认会将资源画像、精细化调度等功能的监控指标以Prometheus的格式对外透出。若您配置组件时开启了ACK-Koordinator开启Prometheus监控指标选项并使用了阿里云Prometheus服务，这些指标将被视为[自定义指标](products/arms/documents/prometheus-monitoring/basic-metrics.md)并产生相应费用。具体费用取决于您的集群规模和应用数量等因素。建议您在启用此功能前，仔细阅读阿里云Prometheus[Prometheus 实例计费](products/arms/documents/prometheus-monitoring/product-overview/billing-description.md)，了解自定义指标的免费额度和收费策略。您可以通过[用量查询](products/arms/documents/prometheus-monitoring/product-overview/billing-and-usage-query.md)，监控和管理您的资源使用情况。

## 相关文档

- 

ACK支持[GPU](products/ack/documents/topology-aware-gpu-scheduling.md)[拓扑感知调度](products/ack/documents/topology-aware-gpu-scheduling.md)，可在节点的GPU组合中选择具有最优训练速度的组合。

- 

可将集群中已分配但未使用的资源量化并提供给低优先级任务使用，以实现[动态资源超卖](products/ack/documents/ack-managed-and-ack-dedicated/user-guide/dynamic-resource-overcommitment.md)。

[上一篇：启用CPU Burst性能优化策略](products/ack/documents/ack-managed-and-ack-dedicated/user-guide/cpu-burst.md)[下一篇：启用动态资源超卖](products/ack/documents/ack-managed-and-ack-dedicated/user-guide/dynamic-resource-overcommitment.md)
