...
集群版本为v1.14及以上版本。如需升级集群，请参见[手动升级集群](products/ack/documents/ack-managed-and-ack-dedicated/user-guide/update-the-kubernetes-version-of-an-ack-cluster.md)。
...
如果您需要使用巡检报告功能，请完成日志服务指定logproject（当前集群logtail-ds组件所使用的logproject）的RAM授权，确保当前RAM用户拥有日志服务指定logproject的数据读取权限，否则会出现权限不足无法查看巡检报告的问题。具体操作，请参见[RAM](products/sls/documents/use-custom-policies-to-grant-permissions-to-a-ram-user.md)[自定义授权示例](products/sls/documents/use-custom-policies-to-grant-permissions-to-a-ram-user.md)。
...
巡检组件security-inspector本身不计费，但会占用您的Pod资源。组件介绍及变更记录，请参见[security-inspector](products/ack/documents/product-overview/security-inspector.md)。
...
默认巡检所有支持的检查项，您可以在配置巡检页面右上方，单击巡检配置，然后配置巡检时执行的检查项。更多信息，请参见[检查项](products/ack/documents/ack-edge/security-and-compliance/use-the-inspection-feature-to-detect-security-risks-in-the-workloads-of-an-ack-cluster.md)。
...
如需进一步提升Pod安全性，可启用安全策略管理功能。更多信息，请参见[启用安全策略管理](products/ack/documents/ack-managed-and-ack-dedicated/security-and-compliance/configure-and-enforce-ack-pod-security-policies.md)。

- 

如需检查集群维度是否存在潜在风险，例如检查资源配额、资源水位等，可使用集群巡检功能。更多信息，请参见[使用集群巡检](products/ack/documents/ack-managed-and-ack-dedicated/user-guide/work-with-the-cluster-inspection-feature.md)。

[上一篇：容器安全策略规则库说明](products/ack/documents/ack-edge/security-and-compliance/predefined-security-policies-of-ack.md)[下一篇：使用安全监控](products/ack/documents/ack-edge/security-and-compliance/use-security-monitoring-capabilities.md)
