...
在示例中，variable表示作为参数提供给Terraform使用的输入变量，关于参数如何传值，请参见[Variable](https://help.aliyun.com/zh/terraform/variable-introduction)。本示例中参数值请参见[附录](products/ack/documents/ack-edge/developer-reference/authorize-the-default-role-through-terraform-when-using-ack-for.md)，其中，服务角色是必选参数，可选角色请根据实际情况选择。
...
[上一篇：首次开通ACK Edge并授权默认角色](products/ack/documents/ack-edge/developer-reference/activate-ack-edge-for-the-first-time-and-authorize-the-default-role.md)[下一篇：通过Terraform创建ACK Edge集群](products/ack/documents/ack-edge/developer-reference/use-terraform-to-create-an-ack-edge-cluster.md)
