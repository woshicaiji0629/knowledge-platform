...
示例中的<Access-key-id>、<Access-key-secret>请分别替换为RAM用户的AccessKey ID、AccessKeySecret。关于如何创建AccessKey ID和AccessKeySecret，请参见[创建](products/ram/documents/user-guide/create-an-accesskey-pair.md)[AccessKey](products/ram/documents/user-guide/create-an-accesskey-pair.md)，配置项说明以及使用临时访问凭证配置请参见[配置访问凭证](products/oss/documents/developer-reference/oss-connector-configuration.md)。
...
添加OSS Connector相关配置并保存。配置项说明请参见[配置](products/oss/documents/developer-reference/oss-connector-configuration.md)[OSS Connector](products/oss/documents/developer-reference/oss-connector-configuration.md)。
...
以下示例旨在使用PyTorch创建一个手写数字识别模型。该模型使用由[OssMapDataset](products/oss/documents/developer-reference/ossmapdataset.md)构建的MNIST数据集，同时借助[OssCheckpoint](products/oss/documents/developer-reference/checkpoint.md)来实现模型检查点的保存和加载。
...
[上一篇：SDK快速入门](products/oss/documents/user-guide/oss-sdk-quick-start.md)[下一篇：在Cursor中使用OSS文档](products/oss/documents/user-guide/use-the-oss-document-in-cursor.md)
